...
def _san(список: list[str]) -> str:
    """Кому годится эта бумага: имена и адреса.

    Имя с «.local» здесь важнее адресов. Адрес робота меняется — от кабеля к
    Wi-Fi, от одной аренды к другой, — и ссылка на пульт умирает вместе с ним.
    Имя не меняется никогда: домашние сети умеют находить по нему сами
    (mDNS, он же Bonjour), так же находятся принтеры и колонки. Без «.local»
    в бумаге браузер на телефоне ругался бы на несовпадение имени, даже найдя
    робота, — а «wheeltec» без хвоста телефон и не ищет.
    """
    имя = socket.gethostname()
    куски = ["DNS:localhost", f"DNS:{имя}"]
    if not имя.endswith(".local"):
        куски.append(f"DNS:{имя}.local")
