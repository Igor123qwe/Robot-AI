"""Камера, воткнутая в самого робота, — один владелец на всех.

Устройство /dev/video* захватывается монопольно: пока его держит один
процесс, второй получает отказ. А просителей у картинки двое и они разные по
характеру — пульт хочет непрерывный поток на экран, голосовой контур хочет
один кадр в тот миг, когда человек спросил «что ты видишь». Если бы каждый
открывал камеру сам, они бы отбирали её друг у друга: пульт открыт в
браузере — робот ослеп, робот посмотрел — у пульта чёрный экран.

Поэтому камерой владеет веб-сервер, и только он. Внутри крутится один ffmpeg,
последний кадр лежит в памяти, а оба просителя берут его оттуда: поток
собирается из тех же кадров, что и снимок. Голосовой службе достаётся
обычный HTTP-запрос к соседнему порту вместо борьбы за устройство.

Захват — тем же способом, что звук: отдельным процессом. Причина та же, что в
audio.py, и проверена на этом же роботе: библиотеки, которые «сами знают, как
надо», на нештатном железе отказывают молча, а ffmpeg с явными параметрами
работает и проверяется руками из консоли.
"""

from __future__ import annotations

import glob
import logging
import os
import shutil
import subprocess
import threading
import time
from typing import Iterator

log = logging.getLogger(__name__)

# Начало и конец кадра JPEG. Искать их в потоке безопасно: внутри сжатых
# данных байт 0xFF всегда экранируется нулём, так что случайного «конца
# кадра» в середине картинки не бывает.
НАЧАЛО = b"\xff\xd8"
КОНЕЦ = b"\xff\xd9"

# Сколько держать камеру открытой после последнего запроса. Держать вечно
# нельзя: устройство греется, ест ток и остаётся занятым для всего
# остального. Десять секунд — чтобы пульт с его потоком не передёргивал
# ffmpeg между кадрами, а разовый вопрос «что ты видишь» не оставлял камеру
# включённой до утра.
ПРОСТОЙ = 10.0

# Через сколько молчания считать камеру зависшей и убивать. Исправная отдаёт
# кадры непрерывно, так что столько тишины подряд бывает только у мёртвой.
ЗАВИС_ЧЕРЕЗ = 10.0

# Сколько ждать НОВЫХ кадров в уже начатом потоке, прежде чем признать, что
# картинки не будет. Без этого предела поток молча висел до общего срока:
# заголовки ушли, тело пустое, у зрителя крутится вкладка и ни ошибки, ни
# картинки — то есть худший вид отказа, при котором даже непонятно, что
# сломалось. Лучше оборвать: браузер тут же спросит причину у /camera/status,
# и человек её увидит.
БЕЗ_КАДРОВ = 5.0

# Сколько ждать самый первый кадр. Камера просыпается не мгновенно: UVC
# договаривается о формате, а автоэкспозиция подбирает выдержку.
ПЕРВЫЙ_КАДР = 6.0


def найти_устройство() -> str:
    """Постоянное имя камеры, а не /dev/videoN.

    Номера раздаются в порядке появления при загрузке и меняются местами.
    Для звука это было бы полбеды, а здесь рядом живут номера, за которыми
    вообще не камера: у UVC-камеры их два (картинка и служебные данные), и
    второй на запрос кадра просто молчит. Ссылка в /dev/v4l/by-id
    привязана к самому устройству и переживает любые перетыкания.
    """
    свои = os.environ.get("ROBOT_CAMERA_DEVICE", "").strip()
    if свои:
        return свои
    # index0 — это как раз ветка с картинкой; вторая отдаёт метаданные.
    по_имени = sorted(glob.glob("/dev/v4l/by-id/*-video-index0"))
    if по_имени:
        return по_имени[0]
    видео = sorted(glob.glob("/dev/video[0-9]"))
    return видео[0] if видео else ""


class Камера:
    """Один ffmpeg на всех, последний кадр — в памяти."""

    def __init__(self, устройство: str = "", размер: str = "", частота: int = 0,
                 качество: int = 0) -> None:
        # Откуда брать картинку. Пусто — из своей камеры, как было всегда.
        #
        # Задано — из чужого потока MJPEG по HTTP, и это не прихоть. USB-камеру
        # может держать только ОДИН процесс. Пока пульт открывал её сам, всё
        # было ладно: он брал её по требованию и отпускал через десять секунд
        # простоя. Но детектор людей на BPU обязан смотреть ПОСТОЯННО — в этом
        # его смысл, — и с этого момента устройство занято всегда. Пульт,
        # продолжай он ломиться в /dev/video0, ослеп бы насовсем, и «что
        # видишь» вместе с ним.
        #
        # Поэтому камерой владеет один хозяин (hobot_usb_cam), а остальные
        # берут у него: web_video_server отдаёт тот же ROS-топик обычным
        # потоком MJPEG по HTTP. Наш разбор кадров ищет JPEG-маркеры в потоке
        # байт — а HTTP-поток устроен так же, — поэтому меняется только
        # источник, вся остальная машинерия остаётся на месте.
        self.поток_url = os.environ.get("ROBOT_CAMERA_URL", "").strip()
        self.устройство = устройство or найти_устройство()
        self.размер = размер or os.environ.get("ROBOT_CAMERA_SIZE", "1280x1024")
        self.частота = частота or int(os.environ.get("ROBOT_CAMERA_FPS", "10"))
        self.качество = качество or int(os.environ.get("ROBOT_CAMERA_QUALITY", "6"))
        self._кадр: bytes = b""
        self._номер = 0                 # растёт с каждым кадром
        self._есть_кадр = threading.Condition()
        self._proc: subprocess.Popen | None = None
        self._поток: threading.Thread | None = None
        self._спросили = 0.0
        self._замок = threading.Lock()
        self._беда = ""

    @property
    def источник(self) -> str:
        """Что скармливать ffmpeg: своё устройство или чужой поток."""
        return self.поток_url or self.устройство

    # --- наружу ---------------------------------------------------------
    def доступна(self) -> bool:
        """Есть ли вообще что открывать. Ответ нужен до первого кадра."""
        if shutil.which("ffmpeg") is None:
            return False
        if self.поток_url:
            # Чужой поток проверить заранее нельзя, не сходив по сети, а
            # ходить по сети в проверке «доступна ли» — значит подвесить
            # ответ на время таймаута. Считаем доступным; не ответит — узнаем
            # из ругани ffmpeg, и она попадёт в журнал.
            return True
        if not self.устройство:
            return False
        return os.path.exists(self.устройство)

    def кадр(self, ждать: float = ПЕРВЫЙ_КАДР) -> bytes:
        """Последний кадр. Поднимает камеру, если та спала."""
        self._спросили = time.monotonic()
        self._поднять()
        срок = time.monotonic() + ждать
        with self._есть_кадр:
            while not self._кадр and time.monotonic() < срок:
                self._есть_кадр.wait(0.2)
            return self._кадр

    def поток(self, пределом: float = 0.0) -> Iterator[bytes]:
        """Кадры по мере появления — для показа в пульте.

        Отдаёт именно НОВЫЕ кадры: без сверки по номеру быстрый потребитель
        крутился бы на одном и том же снимке и грузил бы процессор впустую.

        Предела по времени БОЛЬШЕ НЕТ, и это исправление настоящей поломки.
        Раньше стояла минута, после которой раздача просто заканчивалась:
        соединение закрывалось, браузер оставался с последним кадром, и
        человек видел зависшее видео, которое оживало только перезагрузкой
        страницы. Ровно это Игорь и наблюдал.

        Предел был не нужен изначально. Камеру освобождает не он, а сторож: он
        смотрит на self._спросили, которое обновляется прямо в этом цикле, и
        глушит ffmpeg, когда смотреть перестали. То есть предел ничего не
        охранял, зато исправно ломал показ раз в минуту.

        Ноль означает «без предела». Параметр оставлен для проверок, где
        бесконечный поток неудобен.
        """
        self._поднять()
        видел = -1
        конец = time.monotonic() + пределом if пределом else float("inf")
        последний = time.monotonic()
        while time.monotonic() < конец:
            self._спросили = time.monotonic()
            with self._есть_кадр:
                if self._номер == видел:
                    self._есть_кадр.wait(1.0)
                if self._номер == видел or not self._кадр:
                    if time.monotonic() - последний > БЕЗ_КАДРОВ:
                        log.warning("камера: кадров нет %.0f с — обрываю поток"
                                    "%s", БЕЗ_КАДРОВ,
                                    f" ({self._беда})" if self._беда else "")
                        return
                    continue
                видел = self._номер
                кадр = self._кадр
                последний = time.monotonic()
            yield кадр

    @property
    def беда(self) -> str:
        """Почему камера не отдаёт кадры, словами ffmpeg."""
        return self._беда

    def закрыть(self) -> None:
        with self._замок:
            self._убить()

    # --- внутри ---------------------------------------------------------
    def _поднять(self) -> None:
        with self._замок:
            if self._proc is not None and self._proc.poll() is None:
                return
            if not self.доступна():
                self._беда = (f"нет устройства {self.устройство or '/dev/video*'}"
                              if shutil.which("ffmpeg") else "не установлен ffmpeg")
                log.warning("камера: %s", self._беда)
                return
            cmd = ["ffmpeg", "-hide_banner", "-loglevel", "error"]
            if self.поток_url:
                # У чужого потока ни размер, ни частоту не просят: он такой,
                # какой есть, и требования к нему ffmpeg просто отвергнет.
                # Перекодировать тоже незачем — там уже MJPEG, и «-c copy»
                # экономит процессор, который нужен детектору на той же плате.
                cmd += ["-i", self.источник, "-c", "copy", "-f", "mjpeg", "-"]
                log.info("камера: беру поток %s", self.поток_url)
            else:
                cmd += [
                    "-f", "v4l2",
                    # Камера отдаёт готовый MJPEG, и просить его явно важно:
                    # без этого ffmpeg возьмёт сырой поток, который на USB в
                    # таком разрешении не помещается по полосе, и кадры
                    # посыплются.
                    "-input_format", "mjpeg",
                    "-video_size", self.размер,
                    "-framerate", str(self.частота),
                    "-i", self.источник,
                    "-q:v", str(self.качество),
                    "-f", "mjpeg", "-",
                ]
                log.info("камера: открываю %s (%s, %d к/с)",
                         self.устройство, self.размер, self.частота)
            try:
                self._proc = subprocess.Popen(
                    cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            except OSError as e:
                self._беда = f"не смог запустить ffmpeg: {e}"
                log.warning("камера: %s", self._беда)
                return
            self._беда = ""
            self._поток = threading.Thread(target=self._читать, daemon=True)
            self._поток.start()

    def _убить(self) -> None:
        proc, self._proc = self._proc, None
        if proc is None:
            return
        proc.kill()
        try:
            proc.wait(timeout=3)
        except subprocess.TimeoutExpired:
            log.warning("камера: ffmpeg не закрылся")
        with self._есть_кадр:
            self._кадр = b""
            self._есть_кадр.notify_all()

    def _читать(self) -> None:
        proc = self._proc
        if proc is None or proc.stdout is None:
            return
        буфер = b""
        живо = [time.monotonic()]

        def сторожить() -> None:
            """Убить ffmpeg, когда смотреть некому или когда он замолчал.

            Отдельным потоком, а не проверкой внутри цикла чтения, и это
            принципиально: чтение из трубы блокирующее. Проверка, стоящая
            следом за ним, срабатывает только когда данные пришли, — то есть
            ровно тогда, когда она не нужна. Замолчавшая камера запирала бы
            поток навсегда, держа устройство занятым: та же беда, что была с
            arecord у микрофона. Смерть процесса чтение разблокирует.
            """
            while proc.poll() is None:
                time.sleep(0.5)
                простой = time.monotonic() - self._спросили > ПРОСТОЙ
                завис = time.monotonic() - живо[0] > ЗАВИС_ЧЕРЕЗ
                if простой or завис:
                    log.info("камера: %s — отпускаю",
                             "никто не смотрит" if простой else "кадров нет")
                    proc.kill()
                    return

        threading.Thread(target=сторожить, daemon=True).start()
        try:
            while proc.poll() is None:
                # read1, а не read: обычный read ждёт, пока наберётся
                # ровно столько байт, сколько попросили, и на редких или
                # мелких кадрах картинка застревала бы в трубе до заполнения
                # буфера — тем дольше, чем реже кадры. read1 отдаёт то, что
                # уже пришло.
                кусок = proc.stdout.read1(65536)
                if not кусок:
                    break
                живо[0] = time.monotonic()
                буфер += кусок
                # Из буфера достаём ВСЕ целые кадры, а не первый попавшийся:
                # иначе на быстрой камере буфер растёт быстрее, чем мы его
                # разбираем, и картинка отстаёт от жизни на секунды.
                while True:
                    н = буфер.find(НАЧАЛО)
                    к = буфер.find(КОНЕЦ, н + 2) if н >= 0 else -1
                    if н < 0 or к < 0:
                        break
                    кадр, буфер = буфер[н:к + 2], буфер[к + 2:]
                    with self._есть_кадр:
                        self._кадр = кадр
                        self._номер += 1
                        self._есть_кадр.notify_all()
        except Exception as e:            # noqa: BLE001 — обрыв не повод падать
            log.warning("камера: чтение оборвалось (%s)", e)
        finally:
            # СНАЧАЛА убиваем, и только потом читаем ругань. У живого процесса
            # read() на stderr ждёт конца файла — то есть навсегда, потому что
            # конец наступит лишь с его смертью. Сторож обычно успевает убить
            # раньше, но сюда попадают и по исключению из разбора, а там
            # процесс ещё жив. Цена ошибки высокая: поток виснет, ffmpeg
            # держит устройство, поле процесса не обнуляется и последний кадр
            # не стирается — пульт показывает одну застрявшую картинку.
            proc.kill()
            try:
                proc.wait(timeout=3)
            except subprocess.TimeoutExpired:
                log.warning("камера: ffmpeg не закрылся")
            ругань = b""
            if proc.stderr is not None:
                try:
                    ругань = proc.stderr.read() or b""
                except Exception:         # noqa: BLE001
                    ругань = b""
            if ругань:
                # Ругань ffmpeg — единственное, что отличает «занято другим
                # процессом» от «не та карта» и от «нет такого формата».
                # Прятать её значит заставить человека гадать.
                self._беда = ругань.decode("utf-8", "replace").strip().splitlines()[-1]
                log.warning("камера: %s", self._беда)
            with self._замок:
                if self._proc is proc:
                    self._proc = None
            with self._есть_кадр:
                self._кадр = b""
                self._есть_кадр.notify_all()
