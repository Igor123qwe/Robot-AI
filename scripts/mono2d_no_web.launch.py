    ...
    if _есть("hobot_dosod") and модель:
        вещи = [
            Node(
                ...
                output="screen",
                parameters=[
                    # ЕДИНИЦА, А НЕ НОЛЬ. Узел сам печатает расшифровку в
                    # журнал: «feed_type(0:local, 1:sub)». Ноль означает не
                    # «обычный режим», а «читать картинку из файла»: узел
                    # разбирает config/000000160864.jpg при старте, пишет
                    # результат и БОЛЬШЕ НЕ СЛУШАЕТ НИЧЕГО.
                    #
                    # Поймано на живом роботе, и вслепую поймать было нельзя:
                    # узел поднимается, модель грузит, публикатор заводит —
                    # снаружи всё исправно. А кадры из затвора уходят в
                    # пустоту, и поиск вещей не отвечает НИ РАЗУ. Игорь
                    # потратил на это вечер: сначала на сборку пакета,
                    # который и так был, потом на модель, которой не хватало,
                    # — и всё равно ноль ответов.
                    {"feed_type": 1},
