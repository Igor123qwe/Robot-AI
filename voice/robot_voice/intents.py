...
_NEWS = re.compile(
    r"^(?:а\s+)?(?:что\s+)?(?:там\s+)?"
    r"(?:(?:какие|что\s+за|почитай|прочитай|расскажи|зачитай)\s+)?"
    r"(?:новост\w+|нового\s+в\s+мире|происходит\s+в\s+мире)"
    r"(?:\s+(?:в\s+мире|сегодня|дня|на\s+сегодня))?$"
    r"|^что\s+нового(?:\s+в\s+мире)?$")
...
_WHERE_WE_ARE = re.compile(
    r"^(?:мы|я)\s+(?:живем|живу|находимся|нахожусь|сейчас)?\s*"
    r"(?:в|во)\s+([а-яё-]{3,30})$"
    r"|^(?:наш\s+)?город\s+([а-яё-]{3,30})$"
    r"|^мы\s+(?:в|во)\s+([а-яё-]{3,30})$")
