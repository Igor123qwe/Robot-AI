...
def детектор_ошибся(text: str, имя_в_тексте: bool) -> bool:
    """Детектор на потоке услышал имя, а расшифровка говорит иное.

    Два признака, каждого достаточно: фраза обращена к другому помощнику или
    длинна и без единого слова, похожего на имя. Довод целиком — у
    ДЕТЕКТОР_ВРЁТ_ОТ_СЛОВ и ЧУЖИЕ_ПОМОЩНИКИ. Имя в расшифровке снимает вопрос:
    два свидетеля сошлись.
    """
    if имя_в_тексте:
        return False
    ...
    for wav in listener.utterances():
        ...
        try:
            ...
            if listener.wake_heard:
                ...
                if детектор_ошибся(text, снятое is not None):
                    log.info("имя услышал детектор, а расшифровка — нет: "
                             "считаю, что не звали (%r)", text[:80])
