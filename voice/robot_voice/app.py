...
def _listen_loop(cfg: Config, listener: Listener, recognizer: Recognizer,
    ...
    for wav in listener.utterances():
        ...
        if _is_junk(text):
            ...
            # Ложное срабатывание под музыку: детектор услышал имя в самой
            # песне, робот приглушил её и записал пять секунд музыки, которые
            # распознавание вернуло пустыми. Не вернуть громкость здесь —
            # значит оставить музыку тихой до двадцатисекундной страховки, и
            # она будет сама по себе то тише, то громче.
            _вернуть_музыку(player)
