...
# Как часто поток ночного разбора просыпается посмотреть, не пора ли. Десять
# минут: точность здесь не нужна вовсе, а просыпаться чаще значит будить плату
# ради того, чтобы посмотреть на часы.
РАЗБОР_ПРОВЕРЯЕМ_РАЗ_В = 600.0
# И как часто разбор делается на самом деле. Раз в сутки: выводы копятся
# днями, и чаще их пересчитывать не из чего.
РАЗБОР_РАЗ_В = 24 * 3600.0
...
# Слова, которыми не зовут ЧЕЛОВЕКА. Отдельно от _NOT_A_NAME, потому что
# работа другая: тот список отвечает на вопрос «зовут ли сейчас кого-то
# другого», а этот — «годится ли слово человеку в имя». Складывать их в один
# значило бы одной правкой менять оба ответа.
#
# ЗАКРЫТЫЙ КЛАСС, и потому перечислим целиком — в отличие от имён. Живой
# случай: робот спросил «как тебя зовут?», телевизор в этот миг сказал «Что он
# сказал?» — и завёл жильца по имени ЧТО. В журнале: «Очень приятно, Что»,
# следом «голос Что уточнён, фраз в слепке: 1». Соседний список набирался по
# одному слову за беду; здесь взят весь класс сразу, чтобы следующее слово из
# него не завело ещё одного жильца.
_НЕ_ЗОВУТ_ЧЕЛОВЕКА = {
...
# Как часто пробуем привязаться к вчерашней карте. Раз в пять секунд: привязка
# перебирает пометки, а нового в них за долю секунды не появляется — пока робот
# не увидит ещё один предмет, ответ будет тот же самый.
ПРОБУЕМ_ПРИВЯЗКУ = 5.0

# Как часто рассказываем в журнал, что видит компас по стенам. Раз в минуту:
# дрейф курса изнутри не виден вовсе, и единственный способ узнать, работает ли
# поправка, — смотреть, как она растёт. Чаще — утопим журнал, реже — не
# заметим, что компас молчит.
РАССКАЗ_ПРО_КОМПАС = 60.0
...
# Детектор имени на потоке (wake.Spotter) сработал, а полная расшифровка имени
# не содержит. С этого расхождения начинается КАЖДОЕ ложное срабатывание в
# шумной комнате: словарь у детектора сужен до имени и десятка приказов, и в
# живом разговоре рядом он притягивает к «кузя» обычные слова. Живой час с
# робота: десять пробуждений подряд — «Алиса, а что такое Пин по-английски?»,
# «Он даже его не проверил», «Кошмар. Да, вот сиди и делай с ним», «Я себе
# предлагал, она не хочет» — и ни в одном имени нет. Робот отвечал вслух:
# «Pin — это копейка по-английски».
#
# Раньше расшифровке не верили нарочно: запись начиналась почти на самом
# имени, и «зя, вперёд на метр» по тексту не опознать. Теперь предбуфер держит
# имя целиком (audio.ПРЕДБУФЕР_МС), и расшифровка стала свидетелем, которому
# можно верить — но не слепо. Короткую фразу без имени НЕ отбрасываем:
# распознавание могло проглотить имя, и «Привет.» после «Кузя, привет» — живой
# случай из того же журнала. А вот четыре слова и больше без единого похожего
# на имя — это чужой разговор, а не команда с потерянным началом.
ДЕТЕКТОР_ВРЁТ_ОТ_СЛОВ = 4

# Другие помощники, которые живут в той же комнате. Фраза, начатая с их имени,
# не может быть обращением к роботу — сколько бы детектор на потоке ни
# настаивал, что слышал «Кузя». В доме хозяина стоит колонка Алиса, и каждое
# слово ей рисковало стать словом Кузе. Класс закрытый, потому перечислен.
ЧУЖИЕ_ПОМОЩНИКИ = {"алиса", "маруся", "сири", "салют", "джой", "алекса",
                   "гугл", "олег"}

# Сколько ответов подряд окно разговора может держаться одной комнатой.
#
# Окно продлевалось на КАЖДОЙ обработанной фразе — в том числе на той, что
# пришла без имени, то есть само себя. Телевизор говорит чаще, чем окно
# закрывается, и оно не закрывалось никогда. В живом журнале робот отвечает
# подряд на «— Твоя две.», «Пари.», «А!» — обрывки распознавания комнаты, ни
# одного обращения между ними; на «Пари.» он ещё и переспрашивает «А на что?»,
# чем открывает окно снова.
#
# Порог короткий именно потому, что обнуляется именем: живой разговор про этот
# предел не узнает — человек зовёт робота, когда тот не отвечает. Комната имени
# не говорит никогда, поэтому телевизору достаётся ровно столько ответов.
#
# Ноль здесь был бы честнее, но ломает уговоренное: «Кузя» — «Да?» — «поехали»
# это уже вторая фраза без имени, а третья — обычный ответ на встречный вопрос
# робота. Три — самая короткая цепочка, которая их все переживает.
ОТВЕТОВ_БЕЗ_ИМЕНИ = 3
...
# Объявления колёс, которые значат «стою, и вот почему»: так говорит сторож
# пути в Ros._пустить («Стою: впереди что-то справа.»), память слепых
# направлений («Не поеду: сзади человек.») и упор («Стою: кажется, я во
# что-то упёрся. Посмотри, что там.»). Причина — до первой точки.
_СТОП_С_ПРИЧИНОЙ = re.compile(r"^\s*(?:Стою|Не поеду)\s*:\s*(.+?)\s*(?:[.!…]|$)")


def _причина_стопа(text: str) -> str:
    """Из «Стою: причина.» — «причина»; из любой другой реплики — пусто."""
    m = _СТОП_С_ПРИЧИНОЙ.match(text or "")
    return m.group(1) if m else ""


def _объявить(voice: Voice, text: str, **kw) -> int:
    """Объявление колёс — вслух и лицу.

    Лицо умеет «встревожен(причина)» и рисует «стою: причина» вместо глаз —
    и до сих пор этого не звал никто: сторож пути живёт в ros.py, а тот про
    экран не знает и знать не должен. Единственное, что сторож делает
    наружу, — объявляет причину вслух через ros.объявить. Вот на этом
    проводе и стоим: реплика вида «Стою: …» — это и есть тревога, и лицо
    узнаёт о ней из тех же слов, что слышит человек. Экран не имеет права
    врать — а тут он и не может: слова одни и те же.

    Снимается тревога в сторож() из main: первый же круг сторожа, на
    котором путь свободен, зовёт лицу «путь_свободен».
    """
    причина = _причина_стопа(text)
    if причина:
        _лицо(voice, "встревожен", причина)
    return voice.say(text, **kw)
...
def уверенно_расслышали(sure: float | None) -> bool:
    """Достаточно ли уверенно распознана фраза, чтобы по ней ЕХАТЬ.

    Отдельной функцией, а не строкой в цикле, потому что проверить строку в
    середине `_listen_loop` нечем — и это выяснилось мутацией: заменил
    вычисление на `True`, и ни одна из двух тысяч проверок не заметила.
    Сторож в tools при этом проверен хорошо — но он читает то, что сюда
    записали, и если сюда всегда писать «уверенно», сторож становится
    украшением.
    ...
    `None` значит «распознаватель уверенности не сообщает», и это не повод
    отказывать: так работают все пути, кроме основного.
    """
    return sure is None or sure >= UNSURE_BELOW


def окно_держит_комната(имя_слышно: bool, окно_открыто: bool,
                        было: int) -> tuple[int, bool]:
    """Считает ответы без обращения. Возвращает (стало, пора ли закрыть окно).

    Окно продлевалось на каждой обработанной фразе, включая пришедшую БЕЗ
    имени. Комната говорит чаще, чем окно закрывается, — значит оно держит
    само себя и не закрывается никогда. См. ОТВЕТОВ_БЕЗ_ИМЕНИ.

    Считаем только пока окно открыто: вне окна фразы и так уходят в «не мне»,
    и накручивать на них счётчик значило бы закрыть окно ещё до того, как оно
    откроется.
    """
    if имя_слышно:
        return 0, False
...
def зовут_другого(фраза: str) -> bool:
    """Обращаются по имени, и имя не робота.

    «Виктор, как дела у тебя» и «Иди уже, Рома» — живые строки из журнала, на
    которые робот вступал в чужой разговор. Проверять надо только в открытом
    окне: там имя не требуется, и любое обращение звучит как обращение к нему.

    Список _NOT_A_NAME отсекает слова, которыми фразу НАЧИНАЮТ, а не зовут:
    без него «Ладно, поедем» считалось бы обращением к некоему Ладно.
    """
    зовут = _VOCATIVE.match(фраза or "")
    return bool(зовут) and _clean_token(зовут.group(1)) not in _NOT_A_NAME


def зовут_помощника(фраза: str) -> bool:
    """Фраза начата с имени другого помощника: «Алиса, включи свет»."""
    слова = (фраза or "").split()
    return bool(слова) and _clean_token(слова[0]) in ЧУЖИЕ_ПОМОЩНИКИ


def детектор_ошибся(text: str, имя_в_тексте: bool, голос_знаком: bool = True) -> bool:
    """Детектор на потоке услышал имя, а расшифровка говорит иное.

    Имя в расшифровке снимает вопрос: два свидетеля сошлись. Без имени —
    три признака, каждого достаточно: фраза обращена к другому помощнику;
    она длинна (ДЕТЕКТОР_ВРЁТ_ОТ_СЛОВ); голос НЕЗНАКОМ.

    Про незнакомый голос — по журналу за три с половиной часа с музыкой и
    телевизором рядом, уже с длинным предбуфером. Настоящих обращений
    четыре, и во всех четырёх имя в расшифровке есть. Без имени пришло
    одиннадцать фраз — «Не прибрался», «Chickson», «Да», обрывки песен, — и
    ни одна не была обращением, а «Не прибрался» робот ещё и исполнил:
    «поеду и посмотрю, где что стоит». Все одиннадцать — голос, которого
    робот не знает. Домашних он знает по голосу, и им послабление остаётся:
    детектор услышал имя, расшифровка проглотила — верим детектору. Чужой
    голос, который хочет внимания робота, назовёт его по имени, и теперь имя
    попадает в запись целиком.
    """
    if имя_в_тексте:
        return False
    ...
    for н in range(len(предложения) - 1, -1, -1):
        ...
        for i, token in enumerate(tokens[:3]):
            if _sounds_like_wake(_clean_token(token), wake_words, ratio, канон):
                ...
                # ВСЁ, ЧТО СКАЗАНО ПОСЛЕ, — ТОЖЕ КОМАНДА, а не чужой разговор.
                #
                # Здесь бралось только предложение с именем, а хвост
                # выбрасывался, и это стоило живой команды:
                #
                #   услышано: «Кузя, поехали. Иди за мной.»
                #   ушло в разбор: «поехали»
                #
                # Правило «иду за тобой» ждало ровно «иди за мной» и не
                # сработало; «поехали» правилами не разобралось, ушло к
                # модели, та ответила текстом вместо инструмента — и робот
                # сказал «Не понял, куда ехать» через десять с половиной
                # секунд. Со второй попытки, когда Игорь сказал то же самое
                # одним предложением, правило сработало за 1.1 с.
                #
                # Голое «Кузя!» отдельным предложением лечилось этим же
                # приёмом и раньше — просто в отдельной ветке. Двух правил для
                # одного вопроса быть не должно: хвост принадлежит обращению
                # независимо от того, осталось ли что-то в самом предложении с
                # именем.
                хвост = " ".join(предложения[н + 1:]).strip(" ,.!?…—-")
                if хвост:
                    команда = f"{команда}. {хвост}".strip(" ,.!?…—-") \
                        if команда else хвост
                return команда
...
class Watchdog:
    ...
    def _запасной_путь(self) -> None:
        ...
        было_плохо = bool(self._запас_был)
        self._запас_был = стало
        if стало:
            log.info("работаю на запасном: %s", стало)
            self.voice.status(f"на запасном пути: {стало}")
            # ВСЛУХ — ТОЛЬКО НА ПЕРЕХОДЕ «БЫЛО ХОРОШО → СТАЛО ПЛОХО», а не на
            # каждой смене состава бед. Разница видна на живом журнале:
            #
            #   23:14:31 робот: Отвечаю на запасных силах, ПК не отзывается…
            #   23:15:51 робот: Отвечаю на запасных силах, ПК не отзывается…
            #
            # Между ними отвалилось ещё одно звено — распознавание речи, — и
            # строка состояния изменилась. Для робота это была новость, для
            # человека — то же самое во второй раз. Звеньев, зависящих от ПК,
            # три, значит и объявлений было бы три подряд.
            #
            # Человеку важно одно: ПК не отвечает, будет медленнее. Услышав
            # это, он либо включит ПК, либо нет; повторять ему нечего.
            # Подробный состав идёт в пульт и в журнал, где он и нужен.
            if not впервые and not было_плохо:
                self.voice.say("ПК не отвечает, я работаю сам — "
                               "буду отвечать медленнее.")
        elif not впервые:
...
def main() -> None:
    ...
    def перебили(что: str, почему: str = "перебили голосом") -> None:
        ...
        """
        if что == "колёса":
            # Следование гасим ПЕРВЫМ, до stop_motion. Он остановит его и сам —
            # следование заявлено как движитель, — но напишет в журнал общее
            # «остановили». Причина обрыва читается людьми, и «перебили
            # голосом» объясняет случившееся, а «остановили» — нет.
            if ведомый is not None:
                ведомый.хватит(почему)
            if ros is not None:
                try:
                    ros.stop_motion()
                except Exception:               # noqa: BLE001
    ...
    if not дальномер.запустить():
        ...
        def сторож() -> tuple[bool, str]:
            ...
            if not мешает:
                # Путь свободен — тревога с экрана снимается. Зовётся
                # пятнадцать раз в секунду, пока едем; лицу это дёшево, а
                # само оно узнать, что препятствие ушло, не может.
                _лицо(voice, "путь_свободен")
            return (not мешает), мешает

        ros.сторож = сторож
        # «Стою: причина.» — вслух И лицу: экран рисует ту же причину.
        ros.объявить = functools.partial(_объявить, voice)
        # И обратный провод: дальномер должен знать, с какой скоростью робот
        # едет. Порог стопа считается от неё — тормозной путь растёт квадратом
        # скорости, и постоянный порог одновременно запирал робота на ползании
        # и не успевал на полном ходу.
        ros.скорость_дальномеру = дальномер.едем
    ...
    лицо = face_mod.Лицо(ros=ros, player=player, где=дом,
                         ...
                         # Нет ПК или там нет аватара — просто некому слать.
                         аватар_url=cfg.pc_url,
                         # Звук мультика — из той же колонки, что и музыка:
                         # у робота она одна.
                         видео_устройство=cfg.music_device or cfg.spk_device)
    voice.лицо = лицо
    # Звук, который робот кладёт в динамик, — лицу под рот: и своё лицо на
    # экране, и аватар на ПК ведут рот по настоящей громкости, а не синусом.
    speaker.on_pcm = лицо.озвучил
    лицо.start()
    # Движение в кадре камеры — не только человек, которого ведёт детектор,
    # а всё, что шевелится: кошка, штора, свет. Персонаж на это оглядывается.
    лицо.движение = motion.Движение(едет=лицо._едет)
    лицо.движение.start()
    ...
    #
    # ИМЯ ЗДЕСЬ ВАЖНОЕ, и раньше оно было `глаза` — то же самое, что у камеры
    # четырьмя строками выше. Присваивание молча затирало камеру: объект Глаза
    # терял последнюю ссылку и уходил в мусор, а инструмент look_around
    # («что ты видишь») получал Глазомер, у которого метода `посмотреть` нет
    # вовсе. Робот отвечал моделью по памяти вместо того, чтобы посмотреть.
    # Заодно не подключался счёт расхода на картинки — кадры в облако шли
    ...
    кричали_про: dict[int, int] = {}

    def человек_упал(номер: int) -> None:
        """Подтверждённое падение. Зовётся ИЗ ПОТОКА СВЯЗИ С ШАССИ.

        Отсюда и отдельный поток. По тому же потоку приходят отклики колёс и
        уходит «стоп»; заговорить прямо здесь — значит на всю длину реплики
        перестать слышать шасси, а `cancel_motion` ещё и ждёт в нём подтверждения,
        которое сам же и не даст себе принять.
        """
        раз = кричали_про[номер] = кричали_про.get(номер, 0) + 1

        def отработать() -> None:
            # Сначала колёса. Робот, который едет по комнате, где кто-то
            # лежит на полу, опаснее любого молчания.
            if ros is not None:
                try:
                    ros.cancel_motion(почему="человек упал")
                except Exception:               # noqa: BLE001
                    log.exception("падение: не смог остановить колёса")
            voice.status("человек упал")
            if раз == 1:
                voice.say("Кажется, ты упал. Ответь, что с тобой всё хорошо.",
                          loud=True)
            else:
                # Врать про то, чего не умеем, нельзя: помощь робот вызвать
                # не может, и человек должен это знать, а не ждать.
                voice.say("Ты всё ещё лежишь. Сам я помощь вызвать не могу. "
                          "Скажи, что сделать.", loud=True)

        threading.Thread(target=отработать, daemon=True,
                         name="упал").start()

    # Выключено по умолчанию (ROBOT_FALLDOWN=1 включает). Причина не в
    # качестве: пока не отлажено следование, каждая штука, которая сама
    # трогает колёса и сама заговаривает, добавляется к списку того, что может
    # удивить, — и понять, кто именно виноват, становится труднее.
    сторож_падений = (falldown.Сторож(упал=человек_упал)
                      if cfg.сторож_падений else None)
    if сторож_падений is None:
        log.info("сторож падений выключен (ROBOT_FALLDOWN=1 включает)")

    def детекция(сообщение) -> None:
        """Одно сообщение детектора — двум потребителям.

        Подписка в ros.py одна, и раздаём мы её здесь, а не заводим вторую:
        два подписчика на один топик — это два разбора одного и того же json
        и вдвое больше работы на плате ради нуля новых сведений.
        """
        пеленг.принять(сообщение)
        if сторож_падений is not None:
            сторож_падений.принять(сообщение)

    if ros is not None:
        ros.людей_видно = детекция
    # Карта памяти вокруг робота: дальномер плюс колёса плюс время.
    #
    # Она была написана, проверена — и НИ К ЧЕМУ НЕ ПОДКЛЮЧЕНА. Это нашёл
    # аудит движения, и это худший род мёртвого кода: файл с подробным
    # объяснением, зачем он нужен, и ни одной строки, которая бы его звала.
    #
    # Нужна она ровно за тем, чего у робота нет физически, — за обзором.
    # Дальномер видит шестьдесят градусов вперёд, а корпус при вращении
    # заметает круг; сзади и по бокам робот слеп совсем. Стул, который был
    # перед ним секунду назад, никуда не делся оттого, что робот отвернулся:
    # он остаётся в карте и отвечает на вопрос «что вокруг» настоящим числом
    # вместо прежней догадки «впереди тесно — значит, наверное, и сбоку».
    #
    # Кормим её сеткой БЕЗ ПОЛА. Сырая дала бы ту же беду, что и следованию:
    # при наклоне датчика вниз ряды 2-5 упираются в пол на 0.3-0.9 м, и карта
    # мгновенно обросла бы стеной вплотную к роботу — навсегда и со всех
    # сторон, по которым он проехал.
    # Пометки и постоянная карта заводятся ЗДЕСЬ, до подписки на дальномер,
    # а не там, где их раздают. Обработчик сетки начинает работать сразу
    # после подписки, и обе эти вещи ему нужны с первого же кадра.
    #
    # Пометки — мебель и трудные места; они не тускнеют и переживают
    # перезапуск. План квартиры — то, что копится месяцами: быстрая карта
    # забывает половину за тридцать пять секунд и правильно делает, а стены
    # за сутки не двигаются.
    ...
    план = atlas_mod.Атлас(cfg.data_dir / "квартира.json")

    карта = mapping.Карта() if дальномер else None

    # КОМПАС ПО СТЕНАМ — ОДИН НА ВСЕХ, и заводится он здесь, до первого, кто
    # спросит позу робота.
    #
    # Это не мелочь размещения. Компас правит КУРС, а значит вводит вторую
    # систему отсчёта: исправленную. Карта квартиры строится в ней. Всякий,
    # кто спросит сырую одометрию и полезет с ней в эту карту, окажется
    # повёрнут относительно неё ровно на накопленную поправку — а она растёт.
    #
    # Первая редакция завела компас внутри сборщика карты, и на этом раскол
    # и вышел: карта в исправленных координатах, обход спрашивает её в сырых,
    # пеленги предметов пишутся в сырых. Всё по отдельности выглядело верно.
    #
    # Поэтому источник позы ровно один: компас.поза(ros.odom).
    компас = compass_mod.Компас()
    if карта is not None:
        # Куда выкладывать карту для пульта. /run — память, и это важно:
        # писать четырнадцать тысяч клеток на карточку три раза в секунду
        # значило бы стирать её ресурс ради того, что живёт полсекунды.
        # Каталога /run/robot-ai нет на машине без systemd — там запасной
        # путь рядом с сеткой дальномера, и пульт умеет смотреть в оба.
        куда_карту = Path(mapping.ФАЙЛ_КАРТЫ)
        try:
            куда_карту.parent.mkdir(parents=True, exist_ok=True)
        except OSError:
            куда_карту = Path(mapping.ЗАПАСНОЙ_ФАЙЛ_КАРТЫ)
            log.info("карта пойдёт в %s: каталога в /run нет", куда_карту)

        # Куда выкладывать план квартиры для пульта. Рядом с быстрой картой и
        # по той же причине: /run — это память, а картинка нужна ровно пока
        # открыт пульт. Сама карта при этом живёт на карточке, отдельно.
        куда_план = Path(atlas_mod.ФАЙЛ_ПОКАЗА)
        try:
            куда_план.parent.mkdir(parents=True, exist_ok=True)
        except OSError:
            куда_план = Path(atlas_mod.ЗАПАСНОЙ_ФАЙЛ_ПОКАЗА).expanduser()

        def копить_карту() -> None:
            прошлый = 0.0
            пробовал_привязаться = 0.0
            рассказывал = 0.0
            while True:
                time.sleep(0.1)
                try:
                    взгляд = дальномер.взгляд()
                    поза = ros.odom if ros is not None else None
                    if поза is None or not взгляд.свежий:
                        continue
                    if взгляд.когда == прошлый:
                        continue        # тот же кадр, второй раз не считаем
                    прошлый = взгляд.когда
                    чистая = взгляд.сетка_без_пола
                    if чистая is None:
                        # Плоскость не нашлась — пол от предмета не отличить.
                        # Записать сырое значило бы обнести робота стеной из
                        # собственного пола, и стереть её потом нечем.
                        continue
                    # КУРС ПРАВИМ ПО СТЕНАМ, И ДО ВСЕГО ОСТАЛЬНОГО.
                    #
                    # Гироскоп уплывает на градус-другой в минуту, и за
                    # двадцатиминутный обход набегает до полукруга. На карте
                    # двадцать метров стороной это превращает прямой коридор в
                    # веер: стены расходятся на метры, и никакая навигация не
                    # спасает, потому что она работает на данных, которые уже
                    # неверны. Хуже всего, что изнутри дрейф не виден —
                    # робот уверен в своём курсе ровно так же, как в первую
                    # минуту.
                    #
                    # Стены в квартире дают абсолютный ориентир бесплатно:
                    # жильё строят по прямоугольнику. Подробности — в шапке
                    # compass; здесь важно, что поправка идёт В ОБЕ КАРТЫ
                    # сразу, иначе быстрая и постоянная разъедутся между
                    # собой, и это будет хуже общего дрейфа.
                    try:
                        # РЯДЫ КОМПАСА, А НЕ РЯДЫ КАРТЫ, и это не мелочь.
                        #
                        # Компасу нужны СТЕНЫ, то есть вертикальные плоскости.
                        # Нижние ряды смотрят в пол — в живом журнале это видно
                        # прямо: «y6к0=0.32 (луч под 22°)». А `лучи` берёт
                        # МИНИМУМ по рядам, значит с нижними рядами в каждой
                        # колонке победит пол, и компас будет мерить угол
                        # горизонтальной плоскости вместо вертикальной.
                        #
                        # Пол при этом плоскость честная, но в горизонтальном
                        # срезе он даёт не прямую, а кривую, и проверка на
                        # остаток его отвергает. Снаружи это выглядело как
                        # «компас: стен пока не видел» — ровно так он и
                        # отчитывался на роботе, стоя посреди комнаты.
                        #
                        # Здесь стояло mapping.РЯДЫ = (2, 3, 4, 5) — набор,
                        # выбранный руками по тому же верному доводу, но с
                        # границей не там: при нынешнем кронштейне пол попадает
                        # в окно стены и у рядов 3, 4, 5. Из четырёх рядов три
                        # смотрели в пол. Теперь набор ВЫВОДИТСЯ из геометрии
                        # железа, и переставленный кронштейн пересчитает его сам.
                        компас.заметить(поза[2], карта.лучи(
                            чистая, ряды=compass_mod.РЯДЫ_СТЕН))
                        поза = компас.поза(поза)
                        # РАЗ В МИНУТУ — В ЖУРНАЛ. Дрейф изнутри не виден, и
                        # единственный способ узнать, работает ли поправка, —
                        # смотреть, как она растёт. Молчащий компас неотличим
                        # от отсутствующего.
                        сейчас = time.monotonic()
                        if сейчас - рассказывал > РАССКАЗ_ПРО_КОМПАС:
                            рассказывал = сейчас
                            log.info("%s", компас.как_дела())
                    except Exception:           # noqa: BLE001
                        log.debug("компас не сложился", exc_info=True)
                    # Ряды 2-7, а не 2-5: пол уже вычтен, и нижние ряды
                    # теперь содержат не пол, а то, что НА полу и близко —
                    # тапок, кошку, порожек, ногу. Именно это робот и цепляет.
                    карта.добавить(чистая, поза,
                                   ряды=mapping.РЯДЫ_БЕЗ_ПОЛА)
                    # И выкладываем её наружу — пульт рисует то, что робот
                    # помнит. Это единственное окно в его память: дальномер
                    ...
                    # В ПОСТОЯННУЮ КАРТУ ВЛИВАЕМ, ТОЛЬКО ЕСЛИ ПРИВЯЗАНЫ.
                    # Робот включается носом куда попало; слитая вслепую
                    # карта наложится со сдвигом и поворотом, и две-три
                    # поездки превратят план квартиры в кашу. Отказ копить
                    # обратим, испорченная карта — нет.
                    #
                    # Первый запуск считается привязанным: своя система
                    # координат тогда и становится опорной, сравнивать не с
                    # чем и портить нечего.
                    #
                    # А НАЧИНАЯ СО ВТОРОГО НАДО ПРИВЯЗАТЬСЯ, и вот этого
                    # звена не было вовсе: привязаться() существовал и не
                    # звался ниоткуда. Выходило, что карта копилась ровно
                    # один запуск в жизни файла пометок, а дальше молча
                    # переставала. Игорь это и увидел: «карту не сохраняет,
                    # каждый раз заново рисует».
                    привязан = not пометки.с_прошлого_раза
                    if not привязан:
                        сейчас = time.monotonic()
                        if сейчас - пробовал_привязаться > ПРОБУЕМ_ПРИВЯЗКУ:
                            пробовал_привязаться = сейчас
                            привязан = (пометки.привязаться_чем_вижу()
                                        is not None)
                    план.влить(карта, привязан=привязан)
                    план.сохранить()
                    # И ПОКАЗАТЬ ЕГО ЧЕЛОВЕКУ. До сих пор постоянная карта
                    # не показывалась нигде: пульт рисует «карту вокруг», а
                    # та забывчива по замыслу — половина стирается за
                    # тридцать пять секунд. Карта, которую нельзя увидеть,
                    # неотличима от несуществующей.
                    план.выложить(
                        куда_план, поза=карта.поза, пометки=пометки,
                        # Поворот дома — от компаса. По нему план рисуется
                        # схемой: в этом повороте стены квартиры осевые, и
                        # сплошные ряды занятых клеток и есть стены.
                        поворот=(компас.дом or 0.0) if компас else 0.0,
                        почему=("" if привязан
                                else пометки.чего_не_хватает_для_привязки()))
                except Exception:               # noqa: BLE001
                    log.exception("карта не сложилась")
                    time.sleep(1.0)

        threading.Thread(target=копить_карту, daemon=True).start()

        # Сторож на слепые направления. Дальномер смотрит вперёд, а назад,
        # вбок и в разворот робот двигается наугад — теперь по памяти.
        def сторож_вслепую(x: float, y: float, wz: float) -> tuple[bool, str]:
            try:
                return карта.можно_ехать(x, y, wz)
            except Exception:                   # noqa: BLE001
                # Карта — подсказка, а не опора. Её сбой не должен запирать
                # робота: до сегодняшнего дня слепые направления не
                # проверялись вовсе, и отказ карты возвращает нас ровно туда.
                log.exception("карта не ответила про слепое направление")
                return True, ""

        if ros is not None:
            ros.сторож_вслепую = сторож_вслепую
    ...
    # Путь берём ОТТУДА ЖЕ, ОТКУДА ПОМЕТКИ — из cfg.data_dir, а не из
    # несуществующей landmarks.ФАЙЛ. Здесь стояло именно оно, и робот падал
    # на запуске в вечный перезапуск: модуль такого имени не отдаёт никогда,
    # путь к пометкам ему передают снаружи (строка 918).
    места = search.Места(файл=cfg.data_dir / "места.json")
    ищущий = (search.Поиск(ros, дальномер, план=план, карта=карта,
                           пометки=пометки, компас=компас, места=места,
                           говорить=voice.say)
              if дальномер is not None and ros is not None else None)
    # Поиск отдаём спутнику ссылкой, а не копией: механизм езды по квартире
    # один. Без этой связи состояние ИЩУ было ожиданием на месте — робот,
    # потерявший собаку, стоял столбом до получаса.
    спутник = (dog_mod.Спутник(ros, дальномер, план=план, карта=карта,
                           пометки=пометки, компас=компас, места=места,
                           говорить=voice.say, ищущий=ищущий)
               if дальномер is not None and ros is not None else None)
    ...
    if ведомый is not None and ros is not None:
        ...
        # вовсе: разбор чужого нуля в /cmd_vel начинается с вопроса «а мы
        # вообще едем?», и ответ был «нет».
        #
        # Останавливаем БЕЗ ОЖИДАНИЯ: зовут это в том числе из потока
        # websocket, а он в это время не читает сообщения.
        ros.завести_движитель(
            "следование",
            lambda: ведомый.идёт,
            lambda почему: ведомый.хватит(почему, ждать=False))
    # Жесты рукой. Заводим ПОСЛЕ следования: через ладонь работает остановка,
    # и к первому же жесту должно быть что останавливать.
    def жест_показали(жест: int, _номер: int) -> None:
        """Подтверждённый жест. Зовётся ИЗ ПОТОКА СВЯЗИ С ШАССИ.

        Отсюда отдельный поток, как и у падения: остановка ждёт отклика колёс
        по этому же соединению, а речь занимает голос на всю длину реплики.
        """
        def отработать() -> None:
            if жест == gestures.ЛАДОНЬ:
                # Ровно тот же путь, что у крика «стоп»: сначала колёса, потом
                # речь. Своего пути у жеста быть не должно — иначе одна из
                # двух остановок однажды окажется хуже другой, и выяснится это
                # на роботе.
                # Причину передаём НАСТОЯЩУЮ. В журнале стояло «перебили
                # голосом» там, где перебили ладонью, и по такому журналу
                # нельзя отличить «человек крикнул стоп» от «сеть увидела
                # ладонь» — а это разные беды с разным лечением.
                перебили("колёса", "показали ладонь")
                voice.status("стоп по жесту")
            elif жест == gestures.ТИШЕ:
                перебили("молчи")
                voice.status("молчу")
            elif жест == gestures.ПАЛЕЦ_ВВЕРХ:
                if ведомый is None:
                    return
                # Уже идём — второй раз не начинаем: `начать` в этом случае
                # ответит отказом, а робот произнесёт его вслух и получится,
                # что на поднятый палец он огрызается.
                if ведомый.идёт:
                    return
                voice.say(ведомый.начать())

        threading.Thread(target=отработать, daemon=True, name="жест").start()

    # Тоже выключено по умолчанию (ROBOT_GESTURES=1). Вторая половина
    # выключателя — в scripts/mono2d_no_web.launch.py: без неё плата
    # продолжала бы считать жесты тридцать раз в секунду, а робот просто не
    # слушал бы результат.
    жесты = gestures.Жесты(показал=жест_показали) if cfg.жесты else None
    if жесты is None:
        log.info("жесты выключены (ROBOT_GESTURES=1 включает)")
    elif ros is not None:
        ros.жесты_видно = жесты.принять
    ...
    # Дневник: что робот делал, чем это кончилось и где он тогда был.
    #
    # Навыки — это память о ПРОСЬБАХ: собрал новое действие, сделал, запомнил.
    # Дневник — память о СЕБЕ, и без него робот не знает про себя ничего:
    # каждый вечер просят одно и то же — не заметил; на кухне срывается
    # всегда, а в коридоре никогда — не выучил. Порог наклона дальномера
    # оказался вчетверо ниже собственного шума, и нашлось это только потому,
    # что человек прислал журнал вручную.
    дневник = diary.Дневник(cfg.data_dir / "дневник.jsonl")
    Turn.дневник = дневник
    if ведомый is not None:
        ведомый.дневник = дневник
    дневник.записать("запуск", версия=diary.сборка() or None)

    # Ночной разбор: сложить прожитое в выводы. Считается арифметикой —
    # кластеризация мест, счёт повторов, доля срывов, — и это осознанно.
    # Модель на этом роботе врёт в арифметике, зато хорошо формулирует;
    # значит числа считаем сами, а ей отдаём готовые (см. инструмент
    ...
    def _разбирать_по_ночам() -> None:
        ...
        БЕЗ МОДЕЛИ, и это не экономия. Разговор с моделью идёт через общую
        историю: ночной запрос остался бы в ней и всплыл потом посреди
        утреннего «доброе утро». Ночью считаем числа и сохраняем вывод
        своими словами; красиво его назовёт модель днём, когда человек
        спросит, — там она уже в разговоре и ничего не портит.

        И вслух ночью не говорим вовсе. Разбудить человека выводом о том,
        что робот застревает в углу, — худший способ этот вывод сообщить.
        """
        последний = 0.0
        while True:
            time.sleep(РАЗБОР_ПРОВЕРЯЕМ_РАЗ_В)
            сейчас = time.time()
            if сейчас - последний < РАЗБОР_РАЗ_В or not cfg.is_quiet_now():
                continue
            последний = сейчас
            try:
                разбор.разобрать()
            except Exception:                   # noqa: BLE001
                log.exception("ночной разбор не удался")

    threading.Thread(target=_разбирать_по_ночам, daemon=True).start()
    ...
    вещи = things.Вещи()
    if ros is not None:
        # ПОЗУ БЕРЁМ ИСПРАВЛЕННУЮ, а не сырую. Карта квартиры строится по
        # курсу, поправленному компасом, и пеленг предмета, записанный по
        # сырому, окажется в ДРУГОЙ системе отсчёта — повёрнутой ровно на
        # накопленную поправку. Предмет ляжет на карту косо, и чем дольше
        # робот ездит, тем сильнее.
        ros.вещи_видно = lambda сообщение: вещи.принять(
            сообщение, компас.поза(ros.odom))

    # Пометки раздаём ЗДЕСЬ, а не там, где заводили. Заводятся они рано —
    # обработчику дальномера они нужны с первого кадра, — а раздать их можно
    # только тем, кто уже создан.
    #
    # Ровно на этом я и споткнулся: перенёс создание наверх, а раздачу
    # оставил на месте, и робот упал при запуске на «вещи ещё не
    # существует». Порядок в длинной функции проверяется не глазами.
    ...
    # Каждый инструмент, который зовёт модель, — лицу: персонаж на экране
    # делает то же самое, что робот (едет, включает музыку, ставит таймер),
    # и делает в тот же миг. Иначе слова «еду» и стоящий столбом человечек.
    brain.on_call = lambda имя: _лицо(voice, "делает", имя)
    # …и конец дела: вышло или отказали. Без этого персонаж крутит руль
    # под фразу «ехать не разрешено» — врёт лицом.
    brain.on_done = lambda вышло: _лицо(voice, "сделал", вышло)
...
def _команда_движения(rule, цепочка) -> bool:
    ...
    Цепочка теперь бывает и без единого движения («поставь таймер и включи
    радио») — раньше любая цепочка засчитывалась движением не глядя, и такая
    фраза внутри уже начатого разговора ложно разрешала бы ехать без имени
    (см. cfg.motion_name_once) через полшага после того, где движения и не
    было вовсе.
    """
    if цепочка:
...
class Addressed:
    ...
    def __init__(self) -> None:
        ...
        # прозвучит вслух.
        self.отказ = ""
        # Просили ли уже повторить последнюю неразобранную реплику. Живёт между
        # репликами нарочно: переспрашивать подряд можно только один раз, иначе
        # робот просит повторить телевизор, а тот не слушается. См. _respond.
        self.переспросил = False
...
class Meeting:
    ...
    def __init__(self, pc_url: str) -> None:
        ...
        self.asking = False
        # КОГО ИМЕННО спросили. Без этого имя принималось от кого угодно, кто
        # заговорил следующим, — и на живом роботе им оказался телевизор:
        # спросили один голос, ответил другой, и робот завёл жильца «Что».
        self.спросили_кого = ""
    ...
    def name_is(self, text: str, who: str, voice, people) -> bool:
        ...
        if not self.asking:
            return False
        # ОТВЕЧАТЬ ДОЛЖЕН ТОТ, КОГО СПРОСИЛИ. Спросили один голос, ответил
        # другой — это не знакомство, а совпадение по времени. Так и вышло:
        # робот спросил имя, телевизор сказал «Что он сказал?», и жильцом стало
        # слово «Что». Вопрос при этом НЕ снимаем: человек, которого спросили,
        # может ответить следующей фразой.
        if self.спросили_кого and who != self.спросили_кого:
            log.info("на «как тебя зовут» ответил не тот, кого спросили "
                     "(%s вместо %s) — имя не принимаю",
                     who or "незнакомый голос", self.спросили_кого)
            return False
        self.asking = False
        self.спросили_кого = ""
...
#
# ДВА ПРАВИЛА ВМЕСТО ОДНОГО, и разница не косметическая.
#
# Было одно: `^(?:меня зовут|я)\s+(слово)$`. На живом роботе Игорь сказал
# «Кузя, я тут» — и получил в ответ «Запомнил, Тут», а в журнале осталось
# «личное дело 'Игорь' теперь 'Тут'». Формула «я X» подходит под знакомство
# ровно так же, как под «я тут», «я дома», «я готов», «я устал»: список
# исключений тут не помогает, потому что он открытый.
#
# Разделяем по цене ошибки. ЯВНАЯ формула («меня зовут», «зови меня») —
# знакомство при любых обстоятельствах: так не говорят случайно. А голое
# «я X» принимается ТОЛЬКО у безымянного голоса, которому терять нечего:
# ошибётся — вместо «голос 3» появится «Тут», и это поправимо одной фразой.
# Переименовать уже названного человека голое «я X» больше не может.
_MY_NAME_ЯВНО = re.compile(
    r"^(?:меня\s+зовут|зови\s+меня|мо[её]\s+имя)\s+"
    r"([А-ЯЁа-яёA-Za-z-]{2,20})$")
_MY_NAME_ГОЛОЕ = re.compile(r"^я\s+([А-ЯЁа-яёA-Za-z-]{2,20})$")
...
def _about_people(command: str, who: str, voice, people, meeting) -> bool:
    ...
    m = _MY_NAME_ЯВНО.match(bare)
    if m is None and who and people.nameless(who):
        # Голое «я X» — только для голоса, у которого имени ещё нет.
        m = _MY_NAME_ГОЛОЕ.match(bare)
    if m and who:
...
def _не_имя(слово: str) -> bool:
    """Годится ли слово человеку в имя. True — не годится."""
    н = слово.lower()
    # Наречия на «-но» — «частично», «нормально», «точно», «можно» — именами
    # не бывают, а в комнате звучат постоянно; робот, спросивший «как тебя
    # зовут?» не у того, завёл жильца «Частично». Класс открытый, потому
    # правило по окончанию, а не список. То же правило стоит на ПК
    # (kuzya_pc.годится_в_имя): слепок заводится там и переживает обновление
    # робота. Признак — согласная перед «-но»: у имён («Нино», «Бруно») там
    # гласная, у наречий — согласная.
...
def _person_name(text: str) -> str:
    ...
    if прямо:
        ...
        return "" if _не_имя(имя) else имя
    if len(bare.split()) > 4:
        return ""
...
class Turn:
    ...
    # Куда писать дневник. Ставится один раз в main; None — дневника нет, и
    # всё работает как работало. На классе, а не в __init__: ходов много, а
    # дневник один, и таскать его через все места создания незачем.
    дневник = None
    ...
    def __init__(self, silence: float, записано: float = 0.0) -> None:
        ...
        # Что именно случилось в этом ходе. Копится по дороге, а пишется один
        # раз в конце: одна запись дневника на один обмен с человеком.
        self.про: dict = {}
    ...
    def про_что(self, **поля) -> None:
        """Дописать к записи этого хода. Зовётся по дороге, откуда угодно."""
        for имя, значение in поля.items():
            if значение not in (None, ""):
                self.про[имя] = значение

    def spoke(self, how: str) -> None:
        if self.told:
            return
        self.про.setdefault("как", how)
        self.told = True
    ...
    def разложить(self) -> None:
        ...
        # И одна запись в дневник — тем же местом, что и разбивка в журнал.
        # Место выбрано не случайно: сюда ход приходит один раз и уже со всеми
        # засечками, а значит запись не задвоится и не окажется без времени.
        # Журнал читает человек и построчно; дневник читает робот и складывает.
        if Turn.дневник is not None:
            Turn.дневник.записать("реплика", мс=round(всего),
                                  записано=round(self.записано, 1) or None,
                                  **self.про)
...
def _listen_loop(cfg: Config, listener: Listener, recognizer: Recognizer,
    ...
    awake_until = 0.0
    # Сколько ответов подряд окно держится без единого обращения по имени.
    # См. ОТВЕТОВ_БЕЗ_ИМЕНИ.
    без_имени_подряд = 0
        ...
        try:
            ...
            if _is_junk(text):
                ...
                if слышно and not занят and time.monotonic() < awake_until and not deaf:
                    # Имени в неразобранном нет по определению, значит это тоже
                    # безымянное продление окна — и считается наравне с
                    # остальными. Иначе комната держала бы окно одними
                    # «не расслышал»: см. ОТВЕТОВ_БЕЗ_ИМЕНИ.
                    без_имени_подряд, закрыть = окно_держит_комната(
                        False, True, без_имени_подряд)
                    if закрыть:
                        log.info("не разбираю подряд и имени не слышу — "
                                 "закрываю окно разговора")
                        awake_until = 0.0
                    else:
            ...
            if listener.wake_heard:
                # Имя услышано в ЗВУКЕ. Запись начинается с предбуфера, и имя
                # в неё обычно попадает целиком — тогда снимаем его из
                # расшифровки, чтобы не уехало в команду. Ищем ГДЕ УГОДНО, а
                # не в первых трёх словах предложения: предбуфер захватывает
                # и хвост чужой реплики перед именем, и «слова-то не знает
                # Кузя вперёд» без точки ставит имя шестым словом. Детектор
                # уже сказал, что имя было, — здесь его только находим.
                #
                # Не нашли — детектор и расшифровка разошлись, и дальше
                # решает детектор_ошибся(): чужой помощник или длинная чужая
                # фраза значат, что имени не было, и всё идёт как «не звали».
                снятое = _strip_wake_word(text, cfg.wake_words, cfg.wake_ratio,
                                          канон=_канон(cfg), anywhere=True)
                if детектор_ошибся(text, снятое is not None, голос_знаком=bool(who)):
                    log.info("имя услышал детектор, а расшифровка — нет (голос %s): "
                             "считаю, что не звали (%r)", who or "незнакомый", text[:80])
                    command, по_имени = None, False
                else:
            ...
            # Услышали ли ИМЯ — до всех послаблений ниже, где по_имени ставится
            # по другим признакам. Только имя обнуляет счётчик: см.
            # ОТВЕТОВ_БЕЗ_ИМЕНИ.
            имя_слышно = по_имени
            без_имени_подряд, закрыть = окно_держит_комната(
                имя_слышно, awake, без_имени_подряд)
            if закрыть:
                log.info("окно разговора держится одной комнатой — закрываю, "
                         "позовите по имени (%r)", text[:80])
                awake_until, awake = 0.0, False
            if срез_комнаты:
            ...
            if command is None:
                ...
                if not свои and зовут_другого(command):
                    log.info("зовут не меня (%r)", command)
                    continue
            ...
            addressed.sure = уверенно_расслышали(sure)
            addressed.отказ = ""
            ...
            if turn is not None:
                # Записываем в дневник и текст, и кто сказал. Кто — важнее,
                # чем кажется: «каждый вечер просят одно и то же» имеет смысл
                # только если это один и тот же человек.
                turn.про_что(команда=command,
                             кто=getattr(recognizer, "speaker", "") or None)
            ...
            if answered:
                ...
                for шаг in цепочка:
                    ...
                    if инструмент is None:
                        continue
                    # Между шагами слушаем «стоп»: второй шаг ждёт, пока
                    # доедет первый, и ждёт он в этом самом потоке.
                    if _run_direct(инструмент, шаг.args, voice, turn=turn,
...
def _run_direct(tool, args: dict, voice: Voice, pending: Pending | None = None,
                turn=None, recognizer=None, ros=None) -> bool:
    """Выполняет команду, разобранную правилом, минуя модель.

    True — человек сказал «стоп», пока инструмент работал, и продолжать
    нельзя. Смотри _услышали_стоп: без этой проверки цепочка движений глохла
    ровно там, где «стоп» и говорят.
    """
    # Лицу — что делаем, ДО вызова: персонаж на экране изображает действие
    # в тот же миг, что и робот. Раньше об этом узнавало только от мозга
    # (Brain.on_call), а команды по правилам — «вперёд», «включи радио»,
    # цепочки и навыки — шли мимо: робот ехал, а человечек стоял столбом.
    _лицо(voice, "делает", getattr(tool, "name", ""))
    # Инструмент вызываем до заглушения микрофона: движение стартует сразу, и
    # эти доли секунды робот ещё слышит комнату.
    answer = tool(args)
    # А ВОТ ЗДЕСЬ РОБОТ МОГ ОГЛОХНУТЬ НА СЕКУНДЫ, И ЭТО НАДО ЛОВИТЬ.
    #
    # Инструмент движения возвращается не сразу: Ros._пустить ждёт, пока
    # доедет ПРЕДЫДУЩАЯ поездка, — и ждёт в потоке вызывающего, то есть в
    # главном цикле слуха. На «метр вперёд и метр назад» замерено 3.6 секунды
    # на втором шаге. Всё это время генератор фраз не крутится, и «стоп»
    # обычным путём не проходит: мгновенный слой идёт через тот же генератор,
    # а перебивание живёт только пока робот говорит.
    #
    # Хуже: пропущенный «стоп» не опаздывал, а УНИЧТОЖАЛСЯ. Следующая же
    # реплика вызывает unmute, а тот выбрасывает всё накопленное — вместе с
    # криком, ради которого тридцатисекундный хвост и заведён.
    #
    # Поэтому разбираем хвост ЗДЕСЬ, до say, пока его ещё никто не выбросил.
    if _услышали_стоп(voice, recognizer, ros):
        voice.say("Остановился.")
        return True
...
def _услышали_стоп(voice: Voice, recognizer, ros) -> bool:
    """Не крикнул ли человек «стоп», пока главный цикл стоял в инструменте.

    Пусто в буфере — свободно и мгновенно: разбор заводится только если в
    комнате правда говорили. Платить распознаванием за тишину незачем.
    """
    if recognizer is None or ros is None:
        return False
    try:
        хвост = voice.listener.unheard()
    except Exception:                       # noqa: BLE001
        log.exception("не смог заглянуть в хвост")
        return False
    if not хвост:
        return False
    try:
        return _caught_stop(хвост, recognizer, ros)
    except Exception:                       # noqa: BLE001
        log.exception("сбой при разборе хвоста между шагами")
        return False
...
def _respond(command: str, brain: Brain, voice: Voice,
    ...
    with voice.hold(), contextlib.ExitStack() as stack:
        ...
        # Текст «говорю» на экране — от синтеза, а не от генерации: лицу
        # уходит то предложение, которое ПОШЛО В ДИНАМИК. Модель опережает
        # голос на фразу-две, и текст, поставленный при feed(), бежал бы
        # впереди звука; лицо переключается с «думаю» на «говорю» в тот миг,
        # когда робот и правда заговорил.
        if speech is not None:
            try:
                speech.on_sentence = lambda sentence: _лицо(voice, "говорю", sentence)
            except Exception:                   # noqa: BLE001
                log.debug("реплика не берёт on_sentence", exc_info=True)
        ...
        def произнести(sentence: str) -> None:
            ...
            if sentence.strip(" .!?…").upper() == МИМО:
                # НО НЕ КОГДА РОБОТА ПОЗВАЛИ ПО ИМЕНИ. Тогда это не «говорят
                # не со мной», а ошибка модели, и молчание — худший из
                # возможных ответов: человек обратился и не получил ничего.
                #
                # С живого робота: «проснулся по имени» в 16:39:40, «человек:
                # Найди меня», и следом «модель говорит, что фраза не мне —
                # молчу». Так три раза подряд. Со стороны робот просто не
                # отвечает, и понять почему нельзя ничем, кроме журнала.
                #
                # Четырёхмиллиардная модель на ПК путается в этом слове чаще
                # облачной, и полагаться на её решение там, где ответ уже
                # известен из имени, незачем.
                # НО И ЭТО ВЕРНО ТОЛЬКО ДЛЯ ЗНАКОМОГО ГОЛОСА.
                #
                # Имя робот слышит не только от человека. Телевизор говорит
                # тем же воздухом, и распознаватель имени срабатывает на нём
                # исправно. Дальше телевизионная фраза уходит в модель, та
                # честно отвечает «не мне» — а это правило ей не верило и
                # объявляло вслух «Не разобрал. Повтори, пожалуйста.». За час
                # живого журнала так вышло ДВЕНАДЦАТЬ раз: «Ты на ней, сука,
                # сидел?», «Это похороны», «Особенный» — всё это телевизор, и
                # на каждое робот вслух просил повторить.
                #
                # Различить их можно тем, что у робота уже есть: он узнаёт
                # голос. У девяти из тех двенадцати в журнале стоит «не узнал»,
                # а у настоящих обращений — «Игорь 0.46». Голос из телевизора
                # незнаком по определению: слепки набираются из разговоров, а
                # диктор каждый раз новый.
                #
                # Довод, ради которого правило писалось, при этом цел: беда
                # была в том, что ЧЕЛОВЕК обратился и не получил ничего. Своих
                # робот по-прежнему переспрашивает.
                #
                # И переспрашиваем ОДИН РАЗ ПОДРЯД. Довод выше — про человека,
                # который обратился и не получил ничего; он выкупает первый
                # переспрос, но не второй. Второй подряд означает, что первый
                # ушёл в пустоту: живой человек на «повтори» повторяет, и
                # разбор получается. Подряд же это выходит там, где говорил не
                # человек, — тогда робот просит повторить телевизор, и так до
                # бесконечности, потому что телевизор не слушается.
                кто_говорил = getattr(recognizer, "speaker", "") or ""
                if (addressed is not None
                        and getattr(addressed, "by_name", False)
                        and кто_говорил
                        and not getattr(addressed, "переспросил", False)):
                    log.info("модель сказала «не мне», хотя %s звал по имени — "
                             "не верю ей и переспрашиваю", кто_говорил)
                    мимо[0] = False
            ...
            # ПОРЯДОК ЗДЕСЬ ЗНАЧИМ, и он стоил роботу защиты от самого себя.
            # start_speaking глушит микрофон через voice.quiet() БЕЗ текста —
            # модель к этому мигу ещё ничего не сказала, — а mute() кладёт свой
            # довод в listener.говорю как есть. То есть заглушение ЗАТИРАЕТ то,
            # что робот собрался произнести. Пока говорю(sentence) стоял выше,
            # первое предложение ответа затиралось всегда, и перебивание
            # сравнивало услышанное с пустотой.
            start_speaking()
            говорю(sentence)
            if not speech:
                # Синтеза нет (piper не встал) — звука не будет, и текст на
                # экран ставим здесь: больше некому.
        ...
        try:
            ...
            if придержать:
                ...
                if отказ:
                    ...
                    # ЧИТАЕМ ОТВЕТ МОДЕЛИ ЕЁ ЖЕ ПРАВИЛАМИ, прежде чем сдаться.
                    #
                    # Маленькая модель хуже выбирает инструмент, чем оформляет
                    # вызов, — это известное свойство, и на роботе оно видно
                    # прямо: двадцать три схемы инструментов на семь тысяч
                    # токенов, и на «поехали» модель отвечает СЛОВАМИ
                    # «Поехали! Вперёд.», не позвав drive. Команда в ответе
                    # есть, она просто записана прозой.
                    #
                    # А прозу мы разбирать умеем — теми же правилами, которыми
                    # разбираем человека. Так что вместо «Не понял, куда
                    # ехать» спрашиваем у самого ответа, что в нём обещано, и
                    # если там настоящее движение — выполняем его.
                    #
                    # ГРАНИЦЫ УЗКИЕ, и это важно. Во-первых, сюда попадают
                    # только те ходы, где ЧЕЛОВЕК просил ехать (просят_ехать),
                    # — толковать болтовню робота как команды нельзя.
                    # Во-вторых, берём только ДВИЖЕНИЕ: ни музыку, ни таймеры
                    # из своей прозы не запускаем. В-третьих, сам инструмент
                    # по-прежнему смотрит на «позвали по имени» и «расслышали
                    # уверенно» — эта дорога ничего не обходит, она лишь
                    # переводит обещание в вызов.
                    сказанное = " ".join(придержанное)
                    задумано = intents.parse(сказанное)
                    орудие = (brain.tools.get(задумано.tool)
                              if задумано is not None
                              and задумано.tool in intents.ДВИЖЕНИЕ else None)
                    if орудие is not None:
                        log.info("инструмент не позван, но в ответе %r видно "
                                 "%s(%s) — выполняю", сказанное[:80],
                                 задумано.tool, задумано.args)
                        brain.позвал.append(задумано.tool)
                        придержанное = [str(орудие(задумано.args or {}) or "")]
                    else:
                        log.warning("просили ехать (%r), а инструмент не "
                                    "позван; модель хотела сказать %r — "
                                    "не говорю", command,
                                    сказанное[:120])
                        придержанное = [
                            "Не понял, куда ехать.",
                    ...
                    придержанное = []
                elif (поехал and getattr(ros, "moving", False)
                        and intents.обещает_готово(" ".join(придержанное))):
                    # Робот отчитывается об успехе, а колёса ещё едут. С
                    # живого робота, маршрут «объезд квартиры»:
                    #
                    #   18:06:32 маршрут: шаг 1 из 6
                    #   18:06:33 робот: Объезд квартиры сделан.
                    #   18:06:38 маршрут отменён на шаге 2 из 6
                    #
                    # Инструмент ответил честно — «Делаю объезд квартиры, 6
                    # шагов», — а модель пересказала это прошедшим временем.
                    # Человек услышал «сделан», отвернулся и не увидел, как
                    # робот через пять секунд встал носом в мебель.
                    #
                    # Отличить враньё от честного отчёта по одному тексту
                    # нельзя; можно по колёсам. Едут — значит не сделано, и
                    # вслух идёт то, что вернул инструмент.
                    log.warning("модель объявила дело сделанным (%r), а колёса "
                                "ещё едут — говорю то, что ответил инструмент",
                                " ".join(придержанное)[:120])
                    # Через getattr, а не прямо: заглушки собеседника в
                    # проверках этого поля не имеют, и падать из-за него на
                    # ровном ответе — цена, несоразмерная поводу.
                    правда = getattr(brain, "ответ_инструмента", "")
                    придержанное = [правда] if правда else ["Ещё еду."]
            ...
            # Не ответил — лицу огорчение. Гаснет само через несколько
            # секунд (face.ГАСНУТ_ЧЕРЕЗ), а до того честно показывает, что
            # робот не справился, а не «спокоен» поверх фразы про сбой.
            _лицо(voice, "огорчён")
            start_speaking()
            if speech:
