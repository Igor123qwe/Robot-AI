...
class Remote:
    ...
    def сам_разбираю(self) -> bool:
        ...
        Жило это свойство этажом выше, у Recognizer, — то есть у того, кто на
        ПК не ходит вовсе и никакого `_down_until` не имеет. Обращение к нему
        поднимало AttributeError, а сторож спрашивает через getattr со
        значением по умолчанию — и молча получал «нет». Ровно тот случай,
        когда сломанное неотличимо от исправного: робот честно уходил на своё
        распознавание, честно писал об этом в журнал и никогда не говорил
        вслух, потому что спрашивали не того.
        """
    ...
    def _ask_pc(self, wav_bytes: bytes) -> str | None:
        ...
        try:
            ...
            log.warning("ПК не распознал: %s — перехожу на свои силы", почему(e))
            # Целиком — в отладку. В обычном журнале от неё один вред: три
            # строки внутренностей requests закрывают собой соседние записи,
            # а нужное из них уже вынуто строкой выше.
            log.debug("ПК не распознал, целиком", exc_info=True)
            return None
