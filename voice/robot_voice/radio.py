...
def find(what: str, language: str = "russian") -> tuple[str, str] | None:
    ...
    if not what:
        ...
        # русскую.
        # Фильтр по языку в каталоге проставлен как попало — по нему выпало
        # «Radio Anonymous» вместо чего-нибудь узнаваемого. Страна надёжнее, а
        # тег «pop» отсекает разговорные и церковные станции, которых в топе
        # хватает.
        попытки = (
            {"countrycode": "RU", "tag": "pop", "codec": "MP3"},
            {"countrycode": "RU", "codec": "MP3"},
