...
# косметическое. «Молчи» и «тихо» просят замолчать: человек услышал
...
# закрываю здесь.
#
# «Понял», «ясно», «всё» здесь БЫЛИ и ушли: аудит живого журнала показал, что
# это самые частые «пустые» бытовые подтверждения — их говорят друг другу
# (или ребёнку, собаке), а не роботу, — и распознаватель перебивания не умеет
# отличить, кто именно их произнёс (см. докстроку _слушать_перебивание в
# audio.py). На живом роботе они пять раз подряд обрывали содержательные
# реплики, включая ровно тот момент, когда робот пытался узнать чьё-то имя.
# «Тихо»/«молчи»/«погоди»/«подожди» — императивы, их говорят именно с целью
# кого-то остановить, и ложных срабатываний на них в том же журнале не нашлось.
ПЕРЕБИВАНИЕ = {
    "стоп": "колёса",
    "стой": "колёса",
    "стоять": "колёса",
