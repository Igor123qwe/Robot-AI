...
class Config:
    ...
    # Какую звуковую карту слушать. Пусто — системную по умолчанию, и до
    # появления USB-микрофона этого хватало: карта была одна. С ReSpeaker их
    # становится две, и «по умолчанию» — это уже лотерея: ALSA отдаёт ту, что
    # первой попалась, а вторая молчит. Номер («2») или кусок имени
    # («ReSpeaker») — sounddevice понимает и то, и другое.
    mic_device: str = field(default_factory=lambda: _env("ROBOT_MIC_DEVICE", ""))
    # То же для динамика: уходит в aplay ключом -D. Здесь имя нужно уже в
    # терминах ALSA («plughw:1,0»), а не человеческое: aplay другого не знает.
