...
def _отказ(ответ: str) -> bool:
    """Инструмент отказал? Судим по первым словам его ответа.

    Инструменты отвечают человеческим текстом, а не кодами, и общего признака
    отказа у них нет. Но отказ у всех начинается одинаково — «Не могу»,
    «Нельзя», «Не буду», «Не получилось». Ошибиться тут дёшево: в худшем
    случае персонаж закончит жест «вышло» вместо «отказ», то есть будет ровно
    как раньше.
    """
    начало = (ответ or "").strip().lower()[:40]
    return any(з in начало for з in (
        "не могу", "нельзя", "не буду", "не получ", "не выш", "не наш",
        "не разреш", "отказ", "занят", "ошибка"))
...
class Brain:
    def __init__(self, cfg: Config, tools: list[Tool]) -> None:
        ...
        # Кого позвать, когда модель зовёт инструмент: имя инструмента —
        # и ничего больше. Слушает лицо: персонаж на экране «делает» дело
        # (едет, включает музыку, ставит таймер) в тот же миг, когда робот
        # его делает. Без этого человек видит рот и слышит «Еду», а
        # человечек стоит столбом — общение с ним не складывается.
        self.on_call = None
        # А это — конец дела: вышло или отказали. Персонаж по нему заканчивает
        # жест честно, вместо того чтобы крутить руль под фразу «ехать не
        # разрешено». Необязателен, как и on_call: нет экрана — нет и хлопот.
        self.on_done = None
    ...
    def _params(self, ep: Endpoint, messages: list[dict]) -> dict:  # noqa: C901
        ...
        if self.about and ep.use_cache:
            ...
            #
            # ОГРАНИЧЕНИЕ, И ОНО ИЗВЕСТНОЕ. Роль «system» внутри messages —
            # это не Messages API, это вольность шлюза: наш pc/server
            # (и Ollama/llama.cpp за ним) её принимает и подклеивает к
            # системному блоку, а строгий шлюз, повторяющий Messages API
            # буквально, отвечает 400 на КАЖДЫЙ ход — там роли только user и
            # assistant. Поэтому ветка стоит под «без кэширования», то есть
            # под домашней моделью, а облако идёт веткой выше, системным
            # блоком. Если сюда попадёт строгий собеседник, справку надо
            # будет класть в начало первого user-сообщения — с потерей
            # KV-кэша на её длину, за которую и заведена эта ветка.
                ...
                for call in calls:
                    ...
                    if tool is None:
                        ...
                        if self.on_call is not None:
                            # ДО вызова, а не после: инструмент может ехать
                            # секунды, и персонаж должен тронуться вместе
                            # с колёсами, а не когда они уже встали.
                            try:
                                self.on_call(call.name)
                            except Exception:
                                log.exception("on_call(%s) упал", call.name)
                        output = tool(call.input or {})
                        # А ЭТО — ПОСЛЕ. Персонаж, который взялся за руль, а
                        # ехать ему не разрешили, выглядит вруном: отказ уходит
                        # в динамик, а человечек на экране продолжает крутить
                        # баранку. Теперь у жеста есть конец, и он честный.
                        if self.on_done is not None:
                            try:
                                self.on_done(not _отказ(str(output or "")))
                            except Exception:       # noqa: BLE001
                                log.exception("on_done(%s) упал", call.name)
