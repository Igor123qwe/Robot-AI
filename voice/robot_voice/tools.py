...
def build_tools(ros, timers: Timers, *, speaker=None, notes=None,
    ...
    def дальномер_guard(key: str) -> str | None:
        ...
        if tof is None or _DIRECTIONS.get(key, (0.0, 0.0))[0] <= 0:
            return None
        взгляд = tof.взгляд()
        # Отсутствующий или молчащий датчик движение НЕ запрещает: робот жил
        # без него до сих пор и должен жить дальше. А вот когда он говорит
        # «стена» — это последнее слово, и переспорить его нечем.
        if not взгляд.свежий:
            return None
        можно, почему = взгляд.ехать_можно()
        if можно:
            return None
        return _отказать(f"Не поеду: {почему}.")
