...
def build_tools(ros, timers: Timers, *, speaker=None, notes=None,
    ...
    def stop() -> str:
        ...
        Мультик — впереди музыки в этой же очереди: пока экран занят видео,
        «стоп» почти наверняка про него, а не про колёса или песню.
        """
    ...
    def stop_music() -> str:
        ...
        Видео — впереди музыки по той же причине: голое «выключи» при
        играющем мультике почти наверняка про него.
        """
        if видео is not None and видео.видео_играет():
            return видео.видео_стоп()
    ...
    def music_volume(изменение: str = "тише", уровень: int = 0,
        ...
        Видео — впереди музыки в этой же развилке: «погромче» при играющем
        мультике почти наверняка про него.
        """
    ...
    # music_volume — своим условием, ШИРЕ, чем у остальной музыки: он же
    # регулирует и видео (см. приоритет video > music внутри самой функции),
    # и должен появиться даже там, где player не подключён, а видео есть.
    if player is not None or видео is not None:
        tools.append(Tool(
            name="music_volume",
            description="Сделать музыку (или мультик/видео, если играет он) "
                        "тише или громче. Зови и когда просят убавить звук — "
                        "если что-то играет, речь почти всегда про него. "
                        "Назвали число — передай его в «уровень»; сам "
                        "громкость не меняй и не сообщай о смене, не вызвав "
                        "этот инструмент.",
            input_schema={
                "type": "object",
                "properties": {
                    ...
                    "уровень": {
                        ...
                        "description": "Громкость по десятибалльной: 1 "
                                       ...
                                       "семь», «убавь до двух»). Для "
                                       "видео игнорируется — у него своя "
                                       "шкала, только тише/громче.",
                    },
                    "шаг": {
                        "type": "integer",
                        "minimum": 1,
    ...
    if видео is not None:
        def play_video(что: str = "") -> str:
            return видео.видео_начать(что)

        def video_pause() -> str:
            return видео.видео_пауза()

        def video_resume() -> str:
            return видео.видео_продолжить()

        def video_next() -> str:
            return видео.видео_дальше()
        ...
        tools += [
            Tool(
                name="play_video",
                description="Включить мультик, видео или клип на экране "
                            "(с YouTube). Зови на «включи мультики», «покажи "
                            "мультфильм про Смешариков», «включи клип "
                            "Розовая пантера». Без уточнения — просто "
                            "«мультики». Передавай ровно то, что попросил "
                            "человек, не выдумывай название.",
                input_schema={
                    "type": "object",
                    "properties": {
                        "что": {"type": "string",
                                "description": "Что искать: название "
                                               "мультфильма, персонажа, "
                                               "жанр или клип. Пусто — на "
                                               "твой вкус («мультики»)."},
                    },
                },
                run=play_video,
            ...
            Tool(
                name="video_pause",
                description="Поставить идущий мультик/видео на паузу.",
                input_schema=EMPTY_SCHEMA,
                hidden=True,
                run=video_pause,
            ),
            Tool(
                name="video_resume",
                description="Продолжить мультик/видео после паузы.",
                input_schema=EMPTY_SCHEMA,
                hidden=True,
                run=video_resume,
            ),
            Tool(
                name="video_next",
                description="Следующий мультик/ролик из уже найденных по "
                            "тому же запросу.",
                input_schema=EMPTY_SCHEMA,
                hidden=True,
                run=video_next,
            ...
            Tool(
                name="video_stop",
                description="Выключить мультик/видео и вернуть на экран "
                            "обычного персонажа.",
                input_schema=EMPTY_SCHEMA,
                hidden=True,
                run=video_stop,
