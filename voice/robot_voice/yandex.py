...
def _то_же_имя(имя: str, спросили: str) -> bool:
    """Одно ли это имя с точностью до падежа: «Грот» и «грота» — да.

    Сравниваем по словам и по основам: у русского имени меняется хвост, а не
    начало. Хвостом считаем одну-две буквы — больше уже другое слово.

    На коротких словах основа получается совсем куцей, и «гром» сойдётся с
    «Гротом». Это осознанно: сравнение работает уже поверх выдачи Яндекса, а
    он по «гром» Грота первым исполнителем и не вернёт. Ужать сильнее нельзя —
    «арии» от «Арии» отличается ровно одной буквой при той же длине.
    """
    первые, вторые = имя.lower().split(), спросили.lower().split()
    if not первые or len(первые) != len(вторые):
        return False
    for одно, другое in zip(первые, вторые):
        коротко = min(len(одно), len(другое))
        if abs(len(одно) - len(другое)) > 3 or коротко < 3:
            if одно != другое:
                return False
            continue
        основа = коротко - (2 if коротко > 4 else 1)
        if одно[:основа] != другое[:основа]:
            return False
    return True
...
class Music:
    ...
    def _by_artist(self, client, найдено, limit: int) -> list:
        ...
        Совпадение требуем с точностью до падежа, но не свободнее. Точного
        равенства мало: правило шлёт то, что человек сказал вслух, а вслух
        говорят «включи Грота» и «поставь Короля и Шута» — и вечер уезжал в
        поиск по трекам, то есть в каверы и трибьюты. Свободнее нельзя:
        «поставь спокойную музыку» находит группу «Спокойной ночи», и весь
        вечер играет она.
        """
        ...
        for артист in (артисты or [])[:1]:
            if not _то_же_имя(артист.name or "", спросили):
                return []
            try:
