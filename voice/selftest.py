...
def test_tof() -> None:
    ...
    check("и сказано, почему", "препятствие" in почему, True)
    ...
    check("молчащий датчик запрещает ехать", (можно, почему), (False, "дальномер молчит"))
    ...
    check("и сказано, что это обрыв", "обрыв" in почему, True)
...
def test_tof_guard() -> None:
    ...
    ответ = робот(0.20)["route"]({"steps": [{"move": "вперёд"},
                                            {"turn": "влево"}]})
    check("маршрут в стену не начинаем", "препятстви" in ответ.lower(), True)
