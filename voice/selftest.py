...
def test_slicing() -> None:
    ...
    # Отставание разбора. Раньше это был сплошной счётчик кадров, который
    # ругался каждые 500 штук и не переставал уже никогда: в живом журнале
    # набежало «выброшено 6500 кадров» — число, по которому не понять ни
    # когда это было, ни надолго ли, ни сколько звука на самом деле прошло
    # мимо. Считаем приступами и говорим секундами.
    import logging as _logging

    сказано: list[str] = []

    class Ловец(_logging.Handler):
        def emit(self, record):
            сказано.append(record.getMessage())

    отставший = Pump(types.SimpleNamespace(), keep_seconds=0.2)
    ловец = Ловец()
    аудио_лог = _logging.getLogger("robot_voice.audio")
    аудио_лог.addHandler(ловец)
    try:
        # Догнали почти сразу — молчим: это икота разбора, а не беда.
        отставший._отстаю()
        отставший._догнал()
        check("короткое отставание не тревожит", сказано, [])

        # Две секунды звука мимо ушей — об этом уже стоит сказать, и именно
        # секундами: кадры человек в уме не переводит.
        for _ in range(100):              # 100 кадров по 20 мс
            отставший._отстаю()
        check("пока идёт, не частит", сказано, [])
        отставший._догнал()
        check("сказал один раз", len(сказано), 1)
        check("и сказал секундами звука", "2 с звука" in сказано[-1], True)
        check("а не кадрами", "кадр" in сказано[-1], False)

        # Приступ кончился — счёт начинается заново, а не копится за вечер.
        отставший._отстаю()
        отставший._догнал()
        check("после приступа счёт с нуля", len(сказано), 1)
    finally:
        аудио_лог.removeHandler(ловец)
...
def test_degraded() -> None:
    ...
    # А теперь то же самое, но НАСТОЯЩИМ распознавателем, а не заглушкой с
    # нужным полем. Заглушка выше — ровно та причина, по которой поломка жила
    # незамеченной: свойство «разбираю сам» стояло на классе, который на ПК не
    # ходит вовсе, обращение к нему поднимало AttributeError, сторож спрашивает
    # через getattr со значением по умолчанию — и молча получал «нет». Робот
    # честно уходил на своё распознавание, честно писал об этом в журнал и
    # никогда не говорил вслух.
    слух3 = Remote("http://пк:4000/stt", сделать_модель)
    check("пока ПК жив, сам не разбираю", слух3.сам_разбираю, False)
    слух3._down_until = time.monotonic() + 60
    check("ПК замолчал — робот знает, что разбирает сам",
          слух3.сам_разбираю, True)
    сторож3 = Watchdog(ros, voice, listener, recognizer=слух3, brain=None)
    сторож3._запасной_путь()          # первый заход молчит, только запоминает
    check("и сторож это видит на настоящем распознавателе",
          сторож3._запас_был, "разбираю речь сам")


def test_почему_молчит() -> None:
    """Из ошибки связи должно быть видно, что чинить.

    Три беды выглядят в журнале одинаково — «ПК не отвечает», — а чинятся
    по-разному: одна на ПК за десять секунд, другая в брандмауэре, третья в
    маршрутах. Раньше робот писал то, что отдала библиотека, и разбираться
    приходилось глазами в трёхстрочной простыне от requests.
    """
    import socket
    import urllib.error

    from robot_voice.net import (НЕ_ДОЗВОН, НЕТ_ИМЕНИ, НЕТ_ОТВЕТА, НЕТ_ПУТИ,
                                 НЕТ_СЕТИ, ОТКАЗ, ПРЕДЕЛ, почему)

    section("почему молчит ПК")

    # Обе беды — прямо из журнала живого робота, слово в слово.
    беда = urllib.error.URLError(OSError(113, "No route to host"))
    check("нет пути до ПК", почему(беда), НЕТ_ПУТИ)

    import requests
    import urllib3
    внутри = urllib3.exceptions.ConnectTimeoutError(
        None, "Connection to 192.168.0.23 timed out. (connect timeout=2.0)")
    check("не дозвонился", почему(requests.exceptions.ConnectTimeout(внутри)),
          НЕ_ДОЗВОН)

    # Порт закрыт — это не сеть, а «мост на ПК не запущен». Самая частая
    # причина, и лечится она за десять секунд, если знать.
    check("порт закрыт", почему(ConnectionRefusedError(111, "Connection refused")),
          ОТКАЗ)
    check("сети нет", почему(OSError(101, "Network is unreachable")), НЕТ_СЕТИ)
    check("имя не разрешается", почему(socket.gaierror(-2, "Name or service not known")),
          НЕТ_ИМЕНИ)

    # Дозвон и чтение — разные беды, и путать их нельзя: «не дозвонился» ведёт
    # к брандмауэру и маршрутам, а «не дождался ответа» означает, что ПК жив,
    # слышит и просто не справляется. Идти с этим в сеть — потерянный вечер.
    check("не дождался ответа",
          почему(requests.exceptions.ReadTimeout("Read timed out.")), НЕТ_ОТВЕТА)

    # Номер ошибки теряется по дороге сплошь и рядом: urllib3 заворачивает
    # системную ошибку в свой класс и оставляет только текст.
    class ЧужаяОшибка(Exception):
        pass

    check("узнаём и по одному тексту",
          почему(ЧужаяОшибка("Failed to establish a new connection: "
                             "[Errno 113] No route to host")), НЕТ_ПУТИ)

    # Причина бывает вложена как угодно глубоко: SDK заворачивает httpx, тот —
    # системную ошибку. Разбор должен доставать её из любого слоя.
    внешняя = Exception("Connection error.")
    внешняя.__cause__ = OSError(111, "Connection refused")
    check("достаём причину из глубины", почему(внешняя), ОТКАЗ)

    # И номер важнее класса, а не наоборот. httpx.ConnectError означает и «не
    # дозвонился», и «порт закрыт» — по классу их не различить, по номеру
    # внутри можно. Спроси мы класс первым, человек ушёл бы чинить маршруты
    # вместо того, чтобы открыть окно моста на ПК.
    class ConnectError(Exception):
        pass

    отказ = ConnectError("All connection attempts failed")
    отказ.__cause__ = ConnectionRefusedError(111, "Connection refused")
    check("номер важнее класса", почему(отказ), ОТКАЗ)
    check("а без номера класс всё же говорит своё",
          почему(ConnectError("All connection attempts failed")), НЕ_ДОЗВОН)

    # Незнакомую беду не проглатываем: её тем более нужно увидеть. Но и
    # простыню в журнал не пускаем — она закроет собой соседние строки.
    длинная = почему(Exception("что-то\n  своё " + "и очень длинное " * 40))
    check("незнакомое видно", "что-то своё" in длинная, True)
    check("но в одну строку", "\n" in длинная, False)
    check("и не длиннее предела", len(длинная) <= ПРЕДЕЛ, True)
...
def main() -> int:
    for test in (test_rules, test_numbers, test_when, test_timers,
                 ...
                 test_degraded, test_почему_молчит,
                 test_duck,
                 test_tls,
