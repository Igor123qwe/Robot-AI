...
def test_camera_stream() -> None:
    ...
    try:
        ...
        # И пульт умеет переподключаться, если поток всё-таки оборвался: у
        # живого multipart img.complete равно false, у оборвавшегося — true.
        страница = (Path(__file__).resolve().parent.parent
                    / "web" / "pult.html").read_text()
        check("пульт сторожит застывший поток", "img.complete" in страница, True)
        check("и переподключает его", "включитьВидео(видеоБаза)" in страница, True)
    finally:
        srv.shutdown()
