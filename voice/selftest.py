...
def test_tof() -> None:
    ...
    for н in range(40):
        ...
        for к in range(8):
            ...
            # А это стена в двадцати сантиметрах: близко, но дышит — шум
            # датчика, тень от лампы, дрожание самого робота.
            кадр[4][к] = 0.20 + (н % 8) * 0.02
        кадры.append(кадр)
    свои = tof.свои_зоны(кадры)
    check("кронштейн опознан", sorted({р for р, _ in свои}), [0, 1])
    check("все шестнадцать зон", len(свои), 16)
    # Главное в этой проверке. Признать стену своей деталью — значит перестать
    # её замечать, и это ровно та ошибка, которую потом не видно: робот не
    # тормозит и никаких жалоб не пишет.
    check("живая стена своей не считается", any(р == 4 for р, _ in свои), False)
