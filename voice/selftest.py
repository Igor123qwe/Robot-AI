...
def test_turn_end() -> None:
    ...
    # Имя файла было вшито в скрипт по догадке — и дало ровно то, что и должно
    # было: 404 у человека на роботе и скрипт, который «не работает». В
    # репозитории лежит несколько сборок сразу, имена меняются от версии к
    # версии, и рядом с процессорной есть видеокартная — вчетверо тяжелее и на
    # A55 медленнее. Выбрать не тот файл значит получить замедление там, где
    # мы боролись за миллисекунды.
    #
    # Поэтому список спрашивается у репозитория, а выбирает вот этот код.
    # Проверяем его настоящим содержимым репозитория.
    скрипт = (Path(__file__).resolve().parent.parent
              / "scripts" / "setup_turn_end.sh").read_text(encoding="utf-8")
    выбор = скрипт.split("<<'PY'\n", 1)[1].split("\nPY\n", 1)[0]

    def выбрать(имена):
        готово = subprocess.run(
            [sys.executable, "-c", выбор],
            input=json.dumps({"siblings": [{"rfilename": и} for и in имена]}),
    ...
    # Одна лишь видеокартная — это не «сойдёт», это молча медленнее. Лучше
    # честный отказ с показанным списком, чем тихо выбранный не тот файл.
    _, код = выбрать(["smart-turn-v3.1-gpu.onnx"])
    check("одну лишь gpu-сборку не берём", код, 1)

    # Свежая версия предпочтительнее старой: имена версионированы, и
    # «больше по алфавиту» здесь совпадает с «новее».
