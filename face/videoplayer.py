...
class Проигрыватель:
    ...
    def команда(self, url: str, громкость: int) -> list[str]:
        ...
        if ".m3u8" in url:
            # Прямой HLS-адрес (Rutube отдаёт его через api/play/options):
            # mpv играет его сам, и yt-dlp тут не нужен. Без --ytdl=no mpv
            # всё равно прогонит адрес через yt-dlp — секунды ожидания, а
            # если yt-dlp сломан, то и отказ вместо ролика. Referer — на
            # случай, если раздача CDN проверяет, откуда пришли.
            команда.append("--ytdl=no")
            if "rutube" in url:
                команда.append("--referrer=https://rutube.ru/")
        if self.drm_device:
