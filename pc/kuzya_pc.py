...
def _адреса() -> list[str]:
    """Адреса этой машины в домашней сети — те, что нужно вписать роботу.

    Раньше здесь стояло честное «<адрес этого ПК>», и человек шёл за ним в
    ipconfig. Проблема в том, что ipconfig на машине с Wi-Fi, Ethernet, WSL,
    VirtualBox и VPN показывает шесть адресов, и правильный из них один.
    Ошибиться легко, а выглядит ошибка потом ровно как выключенный ПК: робот
    молча уходит на свои силы и становится медленным.

    Два способа, оба без запроса в сеть. Сокет «наружу» показывает адрес того
    интерфейса, через который машина и правда ходит в сеть, — это лучший
    кандидат. Список по имени машины добавляет остальные: у ПК с проводом и
    Wi-Fi одновременно робот бывает виден только по одному из них.

    Домашние диапазоны идут первыми, но чужие не выбрасываем, а показываем
    следом. Соблазн оставить только 192.168/10/172.16 велик и неправилен:
    домашние сети бывают и на 100.64 от провайдера, и на чём похуже, — а
    «не нашёл адреса» там, где адрес есть, отправляет человека искать
    несуществующую беду. Выбрасываем только то, что не работает никогда:
    петлю и 169.254.x, который означает «DHCP не ответил».
    """
    import socket

    def домашний(адрес: str) -> bool:
        return (адрес.startswith("192.168.") or адрес.startswith("10.")
                or any(адрес.startswith(f"172.{n}.") for n in range(16, 32)))

    найдено: list[str] = []

    def добавить(адрес: str) -> None:
        мёртвый = адрес.startswith("127.") or адрес.startswith("169.254.")
        if адрес and not мёртвый and адрес not in найдено:
            найдено.append(адрес)

    # Пакетов не уходит: UDP-сокет только выбирает маршрут.
    with contextlib.suppress(OSError):
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        try:
            s.connect(("8.8.8.8", 53))
            добавить(s.getsockname()[0])
        finally:
            s.close()
    ...
    with contextlib.suppress(OSError):
        for сведения in socket.getaddrinfo(socket.gethostname(), None,
                                           socket.AF_INET):
            добавить(сведения[4][0])

    # Порядок внутри групп сохраняем: первым нашли — первым и показываем.
    return [а for а in найдено if домашний(а)] + \
           [а for а in найдено if not домашний(а)]


def _брандмауэр(port: int) -> None:
    """Проверить, пустит ли Windows робота на этот порт, и сказать, если нет.

    Самая частая причина «робот не видит ПК» — не сеть и не мост, а Windows:
    порт закрыт, и пакеты робота просто пропадают. Со стороны робота это
    выглядит как молчание — «Connection to 192.168.0.23 timed out», — то есть
    ровно как выключенный компьютер, хотя окно моста открыто и /health на
    самой машине отвечает.

    Ловушек тут две, и вторая обиднее первой. Правило можно завести и всё
    равно остаться закрытым: домашняя сеть после смены роутера часто
    переезжает в профиль «Общедоступная», а правила заводятся для «Частная».

    Разрешение бывает двух видов, и смотреть надо оба. По порту — это правило,
    заведённое руками через netsh. По программе — это то, что заводит сама
    Windows, когда при первом запуске спрашивает «разрешить доступ?»; порт у
    него не указан вовсе. Проверь мы только порт, честно разрешённый мост
    объявлялся бы закрытым, и человек пошёл бы чинить то, что работает.

    Проверка идёт в своём потоке и молчит при любой заминке: это подсказка, а
    не проверка перед запуском, и мешать работе она права не имеет.
    """
    if not sys.platform.startswith("win"):
        return
    import subprocess

    # Кавычки внутри пути не бывают, но экранируем на всякий случай: путь
    # приходит из sys.executable, а не от нас.
    питон = (sys.executable or "").replace("'", "''")
    запрос = (
        "$ErrorActionPreference='SilentlyContinue';"
        f"$p={port};$exe='{питон}';"
        # Нет самих команд (старая Windows, урезанная сборка) — так и скажем,
        # а не выдадим отсутствие ответа за отсутствие правила.
        "if(-not (Get-Command Get-NetFirewallRule -ErrorAction SilentlyContinue))"
        "{'правило: не проверить'}else{"
        "$живые={$_.Enabled -eq 'True' -and $_.Direction -eq 'Inbound'"
        " -and $_.Action -eq 'Allow'};"
        "$поПорту=Get-NetFirewallPortFilter|Where-Object{$_.LocalPort -eq $p}|"
        "Get-NetFirewallRule|Where-Object $живые;"
        "$поПрограмме=Get-NetFirewallApplicationFilter|"
        "Where-Object{$_.Program -eq $exe}|"
        "Get-NetFirewallRule|Where-Object $живые;"
        "if($поПорту -or $поПрограмме){'правило: есть'}else{'правило: нет'}};"
        "Get-NetConnectionProfile|ForEach-Object{'сеть: '+$_.Name+' — '"
        "+$_.NetworkCategory}"
    )
    try:
        готово = subprocess.run(
            ["powershell", "-NoProfile", "-NonInteractive", "-Command", запрос],
            capture_output=True, text=True, timeout=60)
    except Exception:                       # noqa: BLE001 — подсказка, не проверка
        return
    строки = [с.strip() for с in (готово.stdout or "").splitlines() if с.strip()]

    # Формулировка условная, и это не трусость. Правило может быть заведено
    # так, как мы не умеем искать, — тогда всё работает, и пугать незачем.
    # А вот если робот и правда не достучится, эта строка уже будет в окне.
    if "правило: нет" in строки:
        log.warning("разрешающего правила на порт %d не нашёл. Если робот не "
                    "достучится — один раз, от администратора: netsh advfirewall "
                    'firewall add rule name="kuzya" dir=in action=allow '
                    "protocol=TCP localport=%d", port, port)
    общедоступные = [с[len("сеть: "):] for с in строки
                     if с.startswith("сеть: ") and "Public" in с]
    if общедоступные:
        log.warning("сеть «%s» помечена как общедоступная. В этом профиле "
                    "Windows режет входящие мимо правил — переключите её на "
                    "частную, иначе робот не достучится даже с правилом",
                    "», «".join(общедоступные))
...
def main() -> int:
    ...
    if args.host not in ("0.0.0.0", "::"):
        # Слушаем только эту машину — робот не достучится, и делать вид, что
        # достучится, нельзя: адрес в ROBOT_PC_URL он примет молча, а работать
        # это не будет.
        log.info("слушаю только %s — роботу по сети не слышно. Чтобы услышал, "
                 "запускайте без --host либо пробросьте порт через ssh",
                 args.host)
    elif адреса := _адреса():
        log.info("на роботе: ROBOT_PC_URL=http://%s:%d", адреса[0], args.port)
        # Второй адрес — не роскошь: у ПК с проводом и Wi-Fi одновременно
        # робот бывает виден только по одному из них, и по какому именно,
        # заранее не знает никто.
        if len(адреса) > 1:
            log.info("если не достучится — этот ПК виден ещё и как %s",
                     ", ".join(адреса[1:]))
    else:
        log.warning("не нашёл у этого ПК ни одного сетевого адреса — сеть не "
                    "поднята? Смотреть: ipconfig, строка IPv4")
    # В своём потоке: PowerShell отвечает секундами, а сервер должен слушать
    # уже сейчас.
    threading.Thread(target=_брандмауэр, args=(args.port,), daemon=True).start()
