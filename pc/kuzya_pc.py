...
def годится_в_имя(name: str) -> bool:
    ...
    if not name[0].isalpha() or not name.replace("-", "").isalpha():
        return False
    # Наречия на «-но» именами не бывают: «частично», «нормально», «точно»,
    # «можно», «понятно» — ровно то, что звучит в комнате, когда робот спросил
    # «как тебя зовут?» не у того. Живой случай: в базе завёлся жилец
    # «Частично». Класс открытый, списком не закрыть — потому правило по
    # окончанию: «-но» после СОГЛАСНОЙ. Имена на «-но» — «Нино», «Бруно» —
    # держат перед ним гласную, наречия — согласную («частиЧно», «тоЧно»,
    # «моЖно», «нормаЛЬно»). Цена — «Арно»; в русском доме это потеря меньшая,
    # чем жилец на каждое «точно».
    н = name.lower()
    if len(н) >= 4 and н.endswith("но") and н[-3] not in ГЛАСНЫЕ:
        return False
...
class Voiceprints:
    ...
    # За сколько секунд должны прозвучать эти разы, чтобы считаться «подряд».
    #
    # Слово «подряд» стояло в описании и не стояло в коде: кандидат в
    # незнакомцы жил в памяти до перезапуска. Поэтому «подряд» на деле значило
    # «когда-нибудь ещё раз» — хоть через три часа. Два случайных провала
    # хозяина ниже NEW (отвернулся утром, простыл вечером) складывались в
    # нового жильца ровно так же, как две фразы настоящего гостя.
    #
    # Живая беда, ради которой окно заведено: рядом с роботом семь часов
    # играла музыка. Распознавание честно разбирало песни в текст, и робот,
    # пока открыто окно разговора, принимал строки за продолжение фразы
    # человека. Каждая такая строка — кандидат в незнакомцы; за семь часов
    # две похожие сошлись, и в доме завёлся «голос 5» со словами
    # «Время ты куда? Слышь, остановись».
    #
    # Минуты хватает с запасом: настоящий новый человек, которому робот
    # отвечает, говорит следующую фразу секунд через десять. Радио в комнате
    # столько же поводов не набирает.
    ПОДРЯД_ОКНО = 60.0
    ...
    def __init__(self, store: Path) -> None:
        ...
        # Незнакомец, услышанный подряд: слепок, сколько раз и когда в
        # последний раз. См. ПОДРЯД и ПОДРЯД_ОКНО.
        self._чужак = None
        self._чужака_раз = 0
        self._чужака_когда = 0.0
        self._read()
    ...
    def _может_новый(self, mine) -> str:
        ...
        # Кандидат протух: между фразами прошло больше разговора. См. ПОДРЯД_ОКНО.
        if self._чужак is not None and сейчас - self._чужака_когда > self.ПОДРЯД_ОКНО:
            log.info("незнакомец молчал %.0f с — это уже не «подряд», "
                     "начинаю считать заново", сейчас - self._чужака_когда)
            self._чужак, self._чужака_раз = None, 0
...
class Аватар:
    ...
    def __init__(self, часы=time.monotonic) -> None:
        ...
        # Свой пульс: поток, который двигает сценариста независимо от того,
        # смотрит ли кто-нибудь на страницу (см. пульс()).
        self._пульс = None
        self._последнее: dict = {}
    ...
    # --- свой пульс ----------------------------------------------------------
    # Двадцать раз в секунду — как и опрашивает страница. Сценарист обязан
    # тикать САМ, а не от чужого любопытства.
    ТИК = 0.05

    def пульс(self) -> None:
        """Запустить свой тик. Зовётся один раз при старте сервера.

        Без него персонаж двигался, только пока кто-то дёргал /avatar/state:
        страница раз в сотую долю секунды, синтез — ради одного поля эмоции.
        Закрыли вкладку (или она упала с ошибкой) — и сценарист замер на
        полужесте, а вместе с ним встали дыхание, взгляд и отсчёт пауз. Робот
        при этом продолжал получать кадры застывшей страницы и считал, что всё
        хорошо (см. Съёмка.положить).
        """
        if getattr(self, "_пульс", None) is not None:
            return
        self._пульс = threading.Thread(target=self._крутить_тик, daemon=True)
        self._пульс.start()

    def _крутить_тик(self) -> None:
        while True:
            time.sleep(self.ТИК)
            try:
                self._тик()
            except Exception:                       # noqa: BLE001
                log.exception("аватар: тик не прошёл")

    def _тик(self) -> dict:
        """Посчитать состояние заново и запомнить. Здесь и живёт сценарист."""
        с = self._собрать()
        with self._lock:
            self._последнее = с
        return с
    ...
    def состояние(self) -> dict:
        """Что показывать странице. Считает ТИК, здесь только выдача.

        Пока пульс не запущен (самопроверки, короткие сценарии) считаем на
        месте — тогда поведение ровно прежнее.
        """
        if getattr(self, "_пульс", None) is None:
            return self._тик()
        with self._lock:
            return dict(self._последнее) if self._последнее else {}

    def _собрать(self) -> dict:
        with self._lock:
            сейчас = self._часы()
...
class Съёмка:
    ...
    def __init__(self, адрес: str = "", размер: tuple[int, int] = (1280, 800),
        ...
        # С какого времени приходит один и тот же кадр (см. положить).
        self._застыла_с: float | None = None
        self.браузер = ""           # какой в итоге открылся

    # Столько одинаковых кадров подряд — и это уже не «персонаж замер в позе»,
    # а «страница умерла». Пять секунд: самая долгая сценка длится восемь, но
    # и в ней дышит грудь и моргают глаза, а дыхание идёт непрерывно даже во
    # сне. Совсем неподвижной картинка у живой страницы не бывает.
    ЗАСТЫЛА = 5.0

    # --- хранилище: сюда кладёт браузер, отсюда берёт HTTP -----------------
    def положить(self, jpeg: bytes) -> None:
        """Кадр со страницы. Одинаковый кадр — НЕ СВЕЖИЙ кадр.

        Здесь закрыт обман, из-за которого робот мог часами показывать
        мёртвого Марка. Скринкаст Chrome шлёт кадр только на ИЗМЕНЕНИЕ
        картинки, поэтому у застывшей страницы кадров нет вовсе — и ровно на
        этот случай выше (см. ТИШИНА) мы раз в секунду снимаем её обычным
        скриншотом. Скриншот приходит сюда как нормальный кадр, обновляет
        `когда`, и все три сторожа свежести — и здешний СВЕЖИЙ, и робот с его
        ПОТОК_СТАРЕЕТ — считают поток живым. Страница может лежать с
        исключением в консоли, а робот будет верно показывать её последний
        вздох и никогда не переключится на своё лицо.
        Поэтому: кадр, побайтно равный предыдущему, время свежести НЕ
        двигает. Пять секунд такого — и кадр честно стареет, робот уходит на
        домовёнка, а в журнале появляется причина. Сторож стоит здесь, а не
        на роботе: только ПК знает, что страница ОБЯЗАНА шевелиться, — у
        робота повтор кадра штатное дело.
        """
        with self._есть:
            тот_же = jpeg == self._кадр
            self._кадр = jpeg
            self.номер += 1
            if not тот_же:
                self.когда = self._часы()
                self._застыла_с = None
                if self.беда == "страница застыла":
                    self.беда = ""
                    log.info("съёмка аватара: страница снова рисует")
            else:
                если_с = self._застыла_с
                if если_с is None:
                    self._застыла_с = self._часы()
                elif (self._часы() - если_с >= self.ЗАСТЫЛА
                      and self.беда != "страница застыла"):
                    self.беда = "страница застыла"
                    log.warning(
                        "съёмка аватара: картинка не меняется %.0f с — страница "
                        "мертва (смотри консоль браузера). Робот уйдёт на своё "
                        "лицо, вместо того чтобы показывать застывшего Марка",
                        self._часы() - если_с)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _stream(self, model, messages, tools, limit) -> None:
        ...
        try:
            ...
            # Ollama не запущена. Это не сбой в коде, а выключенная программа,
            # и про неё уже сказано при старте отдельной строкой. Трассировка
            # на двадцать строк, повторённая на КАЖДУЮ фразу, не добавляет
            # ничего, кроме того, что настоящие аварии в таком журнале больше
            # не видно: за семь часов у хозяина набежало полсотни одинаковых
            # простыней вокруг одного и того же «Ollama не запущена».
            log.warning("разговор не вышел: Ollama не отвечает (%s). "
                        "Запустите её и оставьте висеть в трее — "
                        "без неё разговаривать не с чем.", e)
            if not started:
...
def main() -> int:
    ...
    # И пусть он живёт сам, а не от чужого любопытства: до этого персонаж
    # двигался, только пока страница спрашивала /avatar/state. Закрыли
    # вкладку — замер на полужесте, и никто об этом не узнавал.
    srv.avatar.пульс()
