...
class Губы:
    """Рот по настоящему звуку: огибающая громкости синтезированной речи.

    Синус вместо рта — честная имитация, но именно имитация: рот хлопает
    ровно, пока звучит фраза, и не совпадает ни с одним слогом. А звук
    рождается здесь же, на ПК (/tts), — грех не взять из него громкость.

    Робот кладёт полученный звук в динамик сразу, друг за другом по
    предложениям, так что и здесь фразы выстраиваются в ту же очередь: начало
    следующей — не раньше конца предыдущей. Сдвиг ЗАДЕРЖКА — на дорогу по
    сети и буфер aplay; точнее без обратной связи от робота не выйдет, и
    сотня миллисекунд на глаз незаметна. Фраза из кэша робота или его
    собственный piper сюда не попадают — тогда остаётся синус.
    """

    ШАГ = 0.02              # секунд на отсчёт огибающей (50 Гц)
    ЗАДЕРЖКА = 0.25         # секунд от синтеза до динамика робота
    ...
    def огибающая(cls, pcm: bytes, rate: int) -> list[float]:
        ...
        открывать рот, иначе персонаж бормочет с закрытым ртом.
        """
        import array
        import math
        ...
        # Степень < 1 поднимает тихие места: рот открыт и на безударных, а не
        # только на пиках, — иначе речь выглядит как редкие щелчки.
...
class Аватар:
    ...
    def ответил(self, текст: str) -> None:
        ...
        if настроение:
            log.info("аватар: ответ звучит как «%s»", настроение)
