...
def main() -> int:
    ...
    if not args.no_avatar_stream:
        try:
            ...
            # Какую страницу снимать — Live2D или 3D — решает config.local.json
            # («движок»: «live2d» или «3d»). Обе живут рядом и обе рабочие:
            # 3D даёт скелет, к которому подходят чужие анимации (Mixamo,
            # .vrma) и физику волос-одежды, Live2D — нарисованные автором
            # кадры. Что лучше — видно только глазами, поэтому выбор оставлен
            # человеку, а не зашит.
