...
def test_scenes() -> None:
    ...
    check("у каждой сценки известный повод",
          [с.имя for с in sc.СЦЕНКИ if с.когда not in sc.СОБЫТИЯ + sc.ФОНЫ], [])
    ...
    н = сц4.кадр(м, 0.1)
    check("музыка началась — «услышал музыку»", н["имя"], "услышал музыку")
    имена = set()
    ...
    check("до конца таймера десять секунд — «считает секунды»", с12["имя"], "считает секунды")
    for i in range(2, 40):
