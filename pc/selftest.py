...
def test_voiceprints() -> None:
    ...
    векторы = {свой: [1.0, 0.0, 0.0], ещё: [0.95, 0.1, 0.0],
               чужой: [0.0, 1.0, 0.0], третий: [0.5, 0.5, 0.707]}
    ...
    # Первая фраза: никого не знаем, голос заводится сам.
    имя, похожесть, метка = who.identify(свой, seconds=3.0)
    check("первого не узнали — некого", имя, "")
    check("но метку дали", bool(метка), True)
    check("завели сам", who.confirm(метка), "голос 1")

    # Вторая фраза того же человека: узнали и уточнили слепок.
    ...
    # Чужой голос: не выдаём за своего и заводим отдельно.
    ...
    who.people.clear()
    who.confirm(who.identify(свой, seconds=3.0)[2])
    who.confirm(who.identify(ещё, seconds=3.0)[2])
