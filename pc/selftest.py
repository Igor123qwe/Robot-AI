    ...
    try:
        ...
        try:
            ...
            try:
                ...
                kuzya_pc.log.warning = старый_warning
            check("путь наружу из avatar/ — это «нет модели», даже если файл на диске есть",
                  настройки3["модель"], "samples/shizuku/shizuku.model.json")
...
def test_scenes() -> None:
    ...
    зажмуренные = sorted(с.имя for с in sc.СЦЕНКИ if доля_закрытых(с) > 0.5)
    check("с закрытыми глазами больше половины времени — только сон, медитация и рожицы",
          зажмуренные,
          ["закрыл лицо ладонями", "корчит рожицу", "медитирует", "подмигнул",
           "показал язык", "притворился спящим", "сопит", "спит"])
    ...
    check("страница: взгляд на человека — В ЕГО сторону (минус: пеленг влево + , а у Cubism + это вправо)",
          ('|| 0) - доля_наклона * 30;' in страница, '|| 0) - доля_наклона;' in страница),
          (True, True))
    ...
    check("3D: взгляд на человека — В ЕГО сторону (минус у пеленга)",
          "const гляжу_x = -доля_наклона" in страница3d, True)
    check("3D: голова поворачивается к человеку, а не только зрачки",
          ("- доля_наклона * ГОЛОВА_К_ЧЕЛОВЕКУ" in страница3d,
