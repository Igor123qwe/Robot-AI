...
def test_voiceprints() -> None:
    ...
    who.confirm(метка)
    # Через .get, а не по ключу: если слепка нет, сторож обязан СКАЗАТЬ об
    # этом, а не рухнуть с KeyError. Падение прогона ничего не доказывает —
    # мутация тогда «ломает код вокруг, а не защиту», и проверить сторожа
    # нечем.
    check("слепок уточнён", who.people.get("голос 1", {}).get("фраз"), 2)
    ...
    # «Подряд» должно значить подряд. Кандидат в незнакомцы жил в памяти до
    # перезапуска, и «два раза подряд» на деле означало «два раза когда-нибудь»
    # — хоть с разницей в три часа.
    #
    # Живая беда: рядом с роботом семь часов играла музыка. Распознавание
    # честно разбирало песни в текст, робот принимал строки за продолжение
    # разговора, и каждая была кандидатом в незнакомцы. За семь часов две
    # похожие сошлись — в доме завёлся «голос 5» со словами песни.
    who.people.clear()
    who.confirm(who.identify(чужой, seconds=3.0)[2])
    check("первая фраза незнакомца — только ждём", len(who.people), 0)
    # Отодвигаем кандидата в прошлое: столько между двумя фразами одного
    # разговора не проходит.
    who._чужака_когда -= who.ПОДРЯД_ОКНО + 1.0
    check("через полчаса это уже не «подряд» — не заводим",
          who.confirm(who.identify(чужой, seconds=3.0)[2]), "")
    check("и никого не завели", len(who.people), 0)
    # Но счёт начат заново, а не сломан навсегда: следующая пара подряд
    # человека заводит.
    check("следующая пара подряд заводит как прежде",
          who.confirm(who.identify(чужой, seconds=3.0)[2]), "голос 1")
    ...
    check("накопили фразы", who.people.get("голос 1", {}).get("фраз"), 2)
    ...
    check("а фразы уцелели", who.people.get("Игорь", {}).get("фраз"), 3)
...
def test_жилец_по_имени_что() -> None:
    ...
    # Живой случай: робот спросил «как тебя зовут?» не у того, в комнате
    # сказали «частично» — и в базе завёлся жилец «Частично».
    for слово in ("Частично", "Нормально", "Точно", "Можно", "Понятно"):
        check(f"«{слово}» — наречие, не имя", годится_в_имя(слово), False)
    for имя in ("Игорь", "Рома", "Анна-Мария", "Igor", "Нино"):
    ...
    store.write_text(json.dumps({
        ...
        "Частично": {"вектор": [0.6, 0.8], "фраз": 2},
    }, ensure_ascii=False), "utf-8")
    who = Voiceprints(store)
    check("жилец «Что» вынесен", "Что" in who.people, False)
    check("и жилец «Частично» тоже", "Частично" in who.people, False)
...
def test_broken() -> None:
    ...
    # Заодно слушаем журнал. Выключенная Ollama — не авария в коде, а
    # невключённая программа, и сказано про неё уже при старте. На живом ПК
    # хозяина она не была запущена семь часов: на КАЖДУЮ фразу в консоль
    # падала трассировка на двадцать строк, и за полусотней одинаковых
    # простыней настоящих сообщений было уже не видно.
    ...
    ловец = Ловец()
    logging.getLogger("кузя-пк").addHandler(ловец)
    ...
    try:
        ...
        srv.shutdown()
        logging.getLogger("кузя-пк").removeHandler(ловец)

    про_разговор = [r for r in записи if "разговор не вышел" in r.getMessage()]
    check("про неудавшийся разговор сказано", len(про_разговор), 1)
    check("но без трассировки: это выключенная программа, а не авария",
          [r.exc_info for r in про_разговор], [None])
    check("и сказано, что делать",
          all("Ollama не отвечает" in r.getMessage() for r in про_разговор), True)
...
def test_scenes() -> None:
    ...
    check("с закрытыми глазами больше половины времени — только медитация и рожицы",
          зажмуренные,
          ["зажмурился от ласки", "закрыл лицо ладонями", "корчит рожицу",
           "медитирует", "подмигнул", "показал язык", "потёр глаз",
           "притворился спящим", "растаял от ласки", "сморщил нос"])
...
def test_измеритель_не_врёт() -> None:
    ...
    # ОТЗЫВЧИВОСТЬ СЧИТАЕТСЯ, А НЕ ЧИСЛИТСЯ. Счётчик потерянных поводов был
    # объявлен в конструкторе сценариста и НЕ УВЕЛИЧИВАЛСЯ НИ РАЗУ — поле и
    # больше нигде. Снаружи «персонаж не отреагировал» и «отреагировал, но
    # сценки не нашлось» выглядят одинаково: человек видит, что на его
    # «спасибо» не ответили, и не знает, глухой робот или занятой.
    сутки = лин.прогнать(суток=1.0, зерно=11)
    check("потерянные поводы и правда считаются, а не всегда ноль",
          сутки["потеряно поводов за сутки"] > 0, True)
    check("…но их немного: почти всё, что случилось, отыграно",
          сутки["потеряно поводов за сутки"] <= 30, True)
    # И МИНУТА РЕЧИ — те секунды, когда на персонажа смотрят в упор.
    check("за минуту речи он не повторяется: жестов много и все разные",
          (сутки["жестов за минуту речи"] >= 12,
           сутки["разных жестов речи за минуту"] >= 10), (True, True))
    # ВЗГЛЯД. Глаз живого не ездит плавно: он стоит на точке доли секунды и
    # ПЕРЕПРЫГИВАЕТ. Прибор поймал прямо: при шаге сетки 0.85 с медиана
    # фиксации была 0.80 с, а в человеческий коридор 200-600 мс попадала одна
    # фиксация из тысячи. Взгляд, стоящий на точке почти секунду, читается как
    # взгляд куклы — и читается мгновенно, раньше всего остального в лице.
    check("взгляд человеческий: фиксации в коридоре 0.2-0.6 с, а не по секунде",
          (0.2 <= сутки["взгляд: медиана фиксации, с"] <= 0.6,
           сутки["взгляд: доля фиксаций 0.2-0.6 с"] >= 0.6), (True, True))
...
def test_его_можно_потрогать() -> None:
    """Экран сенсорный и стоит на высоте детской руки. Его будут трогать.

    Ноль девяносто пять метра выбраны в ТЗ ровно за это. Ребёнок, увидев на
    экране человечка, тянется к нему пальцем ПЕРВЫМ делом — раньше всякого
    разговора. А касание, не попавшее в кнопку, до сих пор просто
    выбрасывалось: персонажа трогают, а он не замечает. Существо, не
    реагирующее на прикосновение, — не существо, сколько сценок ему ни дай.
    """
    section("касание: его трогают, и он это замечает")
    import random as _rnd
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "face"))
    import scenes as sc
    ...
    def тронуть(куда, зерно=77):
        сц = sc.Сценарист(_rnd.Random(зерно))
        for i in range(50):
            сц.кадр(рядом, i / 10, час=14)
        шло = сц._сцена.имя if сц._сцена else ""
        к = сц.кадр(dict(рядом, касание=куда), 5.0, час=14)
        return сц, шло, к

    for куда, повод in sc.КАСАНИЕ_СОБЫТИЕ.items():
        сц, шло, к = тронуть(куда)
        # СРАЗУ, А НЕ ПОТОМ. Отложенная реакция на прикосновение читается не
        # как задумчивость, а как поломка: тронул — ничего, а через три
        # секунды он вдруг хихикнул.
        check(f"тронули «{куда}» — отвечает в тот же кадр и своим поводом",
              (bool(шло), sc.ПО_ИМЕНИ[к["имя"]].когда if к["имя"] else ""),
              (True, повод))

    # ПРИОРИТЕТ 3 ВИДЕН НЕ НА ФОНЕ, А НА СОБЫТИИ. Фоновую сценку перебивает
    # и обычное событие; разница в том, перебьёт ли касание ДРУГОЕ СОБЫТИЕ —
    # например, приветствие, которое идёт две с половиной секунды. Тронуть
    # человечка и ждать, пока он домашется, — это и есть та самая отложенная
    # реакция, которая читается как поломка.
    # ПРИОРИТЕТ 3 ВИДЕН НЕ ТАМ, ГДЕ КАЖЕТСЯ. Перебить фон или обычное
    # событие касание может и с обычным приоритетом: правило перебоя пускает
    # равного (`новая.приоритет >= идущая.приоритет`). Тройка защищает
    # ОБРАТНОЕ — саму реакцию на прикосновение: пока он хихикает от щекотки,
    # обычное событие (сработал таймер, пошёл дождь) её не обрывает. Ребёнок
    # ткнул пальцем, человечек дёрнулся — и на полудвижении переключился на
    # погоду: это ровно то, из-за чего касание перестаёт читаться как ответ
    # ЕМУ.
    сцс = sc.Сценарист(_rnd.Random(79))
    сцс.кадр(dict(рядом, человек=False), 0.0, час=14)
    сцс.кадр(рядом, 0.1, час=14)
    тронули = сцс.кадр(dict(рядом, касание="тело"), 0.5, час=14)
    # Событие берём ОБЫЧНОГО приоритета (музыка — 2): таймер тут не годится,
    # у него приоритет 1, и он не перебил бы касание даже без всякой тройки.
    # Проверка обязана отличать тройку от двойки, а не от единицы.
    заиграла = сцс.кадр(dict(рядом, музыка={"играет": True, "название": "п"}),
                        0.6, час=14)
    check("пока он отвечает на прикосновение, обычное событие его не обрывает",
          (sc.ПО_ИМЕНИ[тронули["имя"]].когда if тронули["имя"] else "",
           заиграла["имя"]), ("ткнули", тронули["имя"]))

    # ЛАСКА И ТЫЧОК — РАЗНЫЕ ВЕЩИ. Доверие растёт только от ласки: оно
    # копится годами и не имеет права расти от того, что в тебя ткнули.
    ласка, _, _ = тронуть("голова")
    тычок, _, _ = тронуть("тело")
    check("погладили — доверие подросло; ткнули — нет",
          (ласка.настроение.значение("доверие") > 0.3,
           тычок.настроение.значение("доверие") <= 0.3), (True, True))
    check("…и любое касание растормошило", 
          ласка.настроение.значение("возбуждение") > 0.5, True)

    # ОДНО ПРИКОСНОВЕНИЕ — ОДНА РЕАКЦИЯ. Поле «касание» живёт один кадр
    # (face.py снимает его сразу после такта); висящее поле означало бы
    # «трогают непрерывно» и превратило бы один тычок в бесконечную щекотку.
    сц = sc.Сценарист(_rnd.Random(78))
    for i in range(50):
        сц.кадр(рядом, i / 10, час=14)
    было = dict(рядом, касание="тело")
    сколько, прошлое = 0, ""
    for i in range(200):
        к = сц.кадр(было, 5.0 + i / 10, час=14)
        if к["имя"] and sc.ПО_ИМЕНИ[к["имя"]].когда == "ткнули" and (
                i == 0 or к["имя"] != прошлое):
            сколько += 1
        прошлое = к["имя"]
    check("палец не убрали — щекотка не бесконечная: реакция одна",
          сколько <= 2, True)


def test_соскучился_после_разлуки() -> None:
    """«Он меня знает» — без имён, без узнавания по голосу и без модели.

    Вернулся через минуту — обычная встреча. Вернулся через полдня — совсем
    другая. Человек читает разницу мгновенно, потому что сам так устроен:
    собака у двери ведёт себя иначе после рабочего дня, чем после выноса
    мусора, и это первое, по чему живое отличают от механизма.
    """
    section("биография: отлучка на кухню и разлука — разные вещи")
    import random as _rnd
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "face"))
    import scenes as sc
    ...
    рядом = {"эмоция": "спокоен", "человек": True, "взгляд": 0.0,
             "музыка": {"играет": False}}
    пусто = dict(рядом, человек=False, взгляд=None)

    def встреча_после(разлука_с):
        сц = sc.Сценарист(_rnd.Random(91))
        for i in range(300):
            сц.кадр(рядом, i * 0.1, час=14)
        t = 30.0
        # Кадры идут ВСЁ время отсутствия: это не пропуск, а жизнь в пустой
        # комнате. Реальный пропуск (ПК держал экран) сценарист обязан
        # перематывать без выдуманных событий — на это свой сторож.
        while t < 30.0 + разлука_с:
            t += 1.0
            сц.кадр(пусто, t, час=int((14 + t / 3600.0) % 24))
        к = сц.кадр(рядом, t + 1.0, час=18)
        return сц, (sc.ПО_ИМЕНИ[к["имя"]].когда if к["имя"] else "")

    _, после_кухни = встреча_после(60.0)
    сц_долгий, после_разлуки = встреча_после(sc.ДОЛГАЯ_РАЗЛУКА + 600.0)
    check("после минуты — обычная встреча, после разлуки — «соскучился»",
          (после_кухни, после_разлуки), ("человек_пришёл", "соскучился"))
    # И это не просто другая сценка — это другое настроение: радость выше.
    check("разлука кончается сильнее, чем отлучка",
          сц_долгий.настроение.значение("радость") > 0.5, True)

    # БИОГРАФИЯ СЧИТАЕТСЯ ПО ЧАСАМ, А НЕ ПО КАДРАМ. Это не «сколько команд
    # выполнил» — такие счётчики интересны хозяину прибора, а не хозяину
    # существа. Это то, из чего складывается жизнь: сколько прожил, сколько
    # времени вы провели вместе, сколько раз тебя гладили.
    био = сц_долгий.биография()
    check("биография считает прожитое, а не вызовы",
          (abs(био["часов_жизни"] - (sc.ДОЛГАЯ_РАЗЛУКА + 600.0 + 30.0) / 3600.0) < 0.1,
           био["встреч"] >= 2, 25.0 < био["секунд_вместе"] < 40.0),
          (True, True, True))
    сцг = sc.Сценарист(_rnd.Random(92))
    for i in range(60):
        сцг.кадр(рядом, i * 0.1, час=14)
    сцг.кадр(dict(рядом, касание="голова"), 6.1, час=14)
    check("поглаживания считаются отдельно от тычков",
          сцг.биография()["поглаживаний"], 1)

    # И ВСЁ ЭТО ПЕРЕЖИВАЕТ ПЕРЕЗАПУСК: биография, которая обнуляется при
    # каждом автообновлении, — это не биография, а счётчик сеанса.
    снимок = сц_долгий.снимок(t=100.0, сейчас=1_700_000_000.0)
    новый = sc.Сценарист(_rnd.Random(91))
    новый.восстановить(снимок, t=0.0, сейчас=1_700_000_000.0)
    check("прожитое переживает перезапуск службы", новый.биография(), био)
    кривой = sc.Сценарист(_rnd.Random(93))
    кривой.восстановить({"биография": {"часов_жизни": -5, "встреч": "много"}},
                        0.0, 1_700_000_000.0)
    check("а дичь в файле биографию не портит: числа только здравые",
          кривой.биография(), {"часов_жизни": 0.0, "встреч": 0,
                               "секунд_вместе": 0.0, "поглаживаний": 0})


def test_вкус_в_доме_переменился() -> None:
    """Выучить легко. Трудно — ПЕРЕДУМАТЬ, когда вкус в доме переменился.

    Существо, которое не может переучиться, не учится, а окостеневает: в
    первый месяц оно сложило мнение и живёт им всю жизнь. Проверяется это
    только сквозным прогоном, потому что мешать тут может что угодно —
    привыкание, отдых сценки, потолок наблюдений, ε-пол.

    Восемь суток хозяин отзывается на жесты руками, потом перестаёт и
    начинает отзываться на всё остальное. Персонаж обязан заметить.

    СВОЕЙ МУТАЦИИ У ЭТОЙ ПРОВЕРКИ НЕТ, И ЭТО НАРОЧНО. Всё, что могло бы её
    сломать — потолок наблюдений, забывание, ε-пол, — сторожится быстрыми
    проверками арифметики, и мутация падает на них первой. Это приёмка
    СИСТЕМЫ целиком: она отвечает на вопрос, доходит ли верная арифметика до
    поведения. Ровно на этом вопросе обучение однажды и провалилось: числа
    были отличные (θ 0.81 против 0.22), а поведение не менялось ни на сколько.
    """
    section("самообучение: вкус в доме переменился")
    import random as _rnd
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "face"))
    import scenes as sc
    ...
    перелом = 8 * 24 * 3600.0
    всего = 16 * 24 * 3600.0
    сыграно, t, нравится = [], 0.0, False
    while t < всего:
        час = int((t / 3600.0) % 24.0)
        дома = 18 <= час < 23
        любит_руки = t < перелом            # ← вкус меняется здесь
        с = {"эмоция": "спокоен", "человек": дома,
             "взгляд": 0.0 if (дома and нравится) else (0.95 if дома else None),
             "услышал": "ага" if (дома and нравится) else "",
             "музыка": {"играет": False}, "ночь": час >= 23 or час < 7}
        к = сцн.кадр(с, t, час=час, минута=int((t / 60.0) % 60), день=1, месяц=9,
                     день_недели=int(t // 86400) % 7)
        имя = к.get("имя") or ""
        окно = сцн._окно
        нравится = bool(окно) and (с_руками(окно["имя"]) == любит_руки)
        if имя and дома and (not сыграно or сыграно[-1][1] != имя):
            сыграно.append((t, имя))
        t += 1.0

    def доля(с_суток, по_сутки):
        свои = [и for когда, и in сыграно
                if с_суток * 86400 <= когда < по_сутки * 86400
                and sc.ПО_ИМЕНИ[и].когда in sc.ФОНЫ]
        return sum(1 for и in свои if с_руками(и)) / len(свои) if свои else 0.0

    начало, выучил, переучился = доля(0, 1), доля(6, 8), доля(14, 16)
    check("сперва выучил вкус дома", выучил > начало + 0.05, True)
    check("а когда вкус переменился — переучился за неделю, и ниже, чем начинал",
          переучился < начало, True)
    # И РЕПЕРТУАР ПРИ ЭТОМ ЦЕЛ. Переучиться, выбросив половину библиотеки, —
    # не «передумал», а «сломался»: человек увидит это раньше любой метрики.
    последние = {и for когда, и in сыграно if когда >= 14 * 86400}
    check("…не выбросив при этом половину библиотеки", len(последние) >= 40, True)
